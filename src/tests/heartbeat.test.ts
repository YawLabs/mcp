...
// success path's request shape, and the two failure modes yaw-mcp swallows
...
describe("reportHeartbeat", () => {
  ...
  it("posts to /api/connect/heartbeat with bearer auth and client info", async () => {
    initHeartbeat("https://yaw.sh/mcp", "tok");
  ...
  it("strips a trailing slash from apiUrl before appending the path", async () => {
    // Defends against a misconfigured YAW_MCP_URL that ends with '/' --
    ...
    expect(String(url)).toBe("https://yaw.sh/mcp/api/connect/heartbeat");
  });
});
