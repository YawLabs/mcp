// Fire-and-forget self-upgrade check, run once per mcph serve startup.
//
// mcph cannot hot-swap its own running code, so "auto-update" means:
...
// @yawlabs/mcph@latest` is a whitelisted, non-destructive command.
//   - npx installs self-heal already: `mcph install` now writes
//     `@yawlabs/mcph@latest`, so `npx` re-resolves the newest version on
...
export async function maybeAutoUpgrade(deps: AutoUpgradeDeps = {}): Promise<void> {
  ...
  if (method === "global-npm") {
    ...
    (deps.spawnImpl ?? defaultSpawn)("npm", ["install", "-g", "@yawlabs/mcph@latest"]);
    return;
  }
  ...
  // visible; the `@latest` client config (written by `mcph install`)
