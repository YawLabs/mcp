...
`install typed` writes typed's own `~/.config/typed/mcp.json` (the same path on every OS, whatever `CLAUDE_CONFIG_DIR` says), which typed ranks above Claude Code's user-scope files -- so it wins over an `mcp` entry Yaw Terminal manages in `~/.claude.json`, while a project's `.mcp.json` still wins over it. It needs a typed CLI newer than 1.5.0; with an older one, use `install claude-code`, whose files typed also reads. `install typed` checks the typed CLI bundle on this machine (`~/.config/typed/typed-cli/cli.mjs`, or `$TYPED_CLI_BUNDLE`) and warns when it cannot read the file yet, naming `typed update`. Like `install claude-code`, it adds `mcp__mcp__*` to `permissions.allow` in Claude Code's user `settings.json`, which typed reads for permissions too. The two share that one grant, so `uninstall` of either keeps it while the other still has its entry, and says so. In a Yaw Mode pane, where `CLAUDE_CONFIG_DIR` is a per-pane overlay whose `settings.json` is discarded with the pane, both commands write the grant to `~/.claude/settings.json` too (augment) -- or, in a fresh pane, say that it goes with the pane and to install from a normal shell to keep it.
