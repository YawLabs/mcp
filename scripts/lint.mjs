 ...
 *
 * On THIS repo the arm64 crash described above is not reproducing today: at
 * the pinned biome 2.4.16 the native arm64 binary exited 0 on three direct runs
 * (2026-09-11). The crash measurements above come from @yawlabs/aws-mcp at
 * 2.4.12; this repo hit its own variant under biome 2.5.x, which v0.72.0 got
 * out of by pinning 2.4.16. The x64 route is used here anyway, so the lint gate
 * does not depend on whether the next biome bump re-breaks the arm64 build.
 *
 ...
 * failure, they are not performance budgets. For scale, biome checks this
 * repo's 190 files in about 1s natively and about 2s as x64 under emulation
 * (measured 2026-09-11).
 */
