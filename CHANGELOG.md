...
## Unreleased -- `install --force` stops keeping the env it promised to overwrite, editing a client config stops rewriting the entry next to ours, and five install surfaces stop stating things that are not so
...
`uninstall`, `doctor`, `install --list` and `import` now also SEE an entry under the other drive-letter spelling of the same directory, which is what an older version wrote. Each of them used to look under one key only: `uninstall --project-dir c:/repo` stripped the `permissions.allow` grant, printed "Claude Code no longer launches yaw-mcp", and left the entry -- which a Claude Code started from a cmd prompt still read -- and a second run then reported "Nothing to do"; `doctor` and `install --list` reported the project as having no yaw-mcp entry at all. `uninstall` now removes the entry from every spelling in one write and names each one it removed, and its closing all-clear is no longer printed over wiring the same run left behind. `doctor` and `install --list` report the entry and name the key it is under: the STATUS column can now read `installed (other drive case)`, with the key itself printed in a note under the table, since that is the only place a full path fits. A script matching that column exactly will see the new text for a project whose entry is under the other spelling. Only the drive letter is folded: two keys differing in any other character, and POSIX and UNC keys, are still separate projects. `doctor --json` carries the key in a new additive field, `clients[].entryProjectKey`.
...
Upgrading from 1.0.0, concretely. A local-scope install made with `--project-dir c:/repo` left two things behind: `projects["c:/repo"].mcpServers.mcp` in `~/.claude.json`, and `mcp__mcp__*` in `<repo>/.claude/settings.local.json`. On 1.0.0 the first `uninstall claude-code --scope local --project-dir c:/... --force` removed only the permission grant, printed "Claude Code no longer launches yaw-mcp" over an entry that still launched it for a cmd.exe session (and for a Git Bash that inherited a lower-case cwd from a cmd prompt), and a second run then said "Nothing to do". One run of this version clears both: the entry under whichever spelling holds it, and the grant.
