...
`yaw-mcp install typed` wires yaw-mcp into the [typed](https://typed.cloud) CLI, and `uninstall`, `install --list`, `--all`, `doctor`, `try`, `import` and shell completion all take the new id. It writes typed's own `~/.config/typed/mcp.json`, not one of Claude Code's files: the same path on every OS, never moved by `CLAUDE_CONFIG_DIR`, and strict JSON, so install refuses to splice into a copy carrying a comment, as it does for Cline. typed ranks that file above Claude Code's user-scope files, so its `mcp` entry wins over one Yaw Terminal manages in `~/.claude.json`, while a project's `.mcp.json` still wins over it. On Windows the entry is a bare `npx`, because typed resolves the `.cmd` shim itself. There is no project scope -- `<project>/.mcp.json` is already Claude Code's, and `install mcp` writes it. It needs a typed CLI newer than 1.5.0; older typed reads only Claude Code's files, and `install claude-code` is the command to run there.

typed reads `permissions.allow` from Claude Code's user `settings.json` (`$CLAUDE_CONFIG_DIR/settings.json` when that is set), so `install typed` adds the same `mcp__mcp__*` grant `install claude-code` adds, to the same file, and typed does not prompt on each yaw-mcp tool call. Install and uninstall now decide whether to touch that grant from the install target, which declares that it patches Claude Code's permissions, rather than from the client being Claude Code. Because the two clients share one grant, `uninstall` stops removing it while the other still has its entry: `uninstall typed` with Claude Code still wired, or `uninstall claude-code --scope user` with typed still wired, leaves the grant and prints one line naming the client that keeps it, `Keeping mcp__mcp__* in <settings.json>: Claude Code (user) still launches yaw-mcp from <.claude.json> and reads that grant.` It also leaves a grant when the other client's config exists but cannot be read. One case changes for a Claude-Code-only setup: `uninstall claude-code --scope project` with your home directory as the project folder and `CLAUDE_CONFIG_DIR` unset, where the project and user `settings.json` are the same file, now keeps the grant while the user-scope entry remains.
