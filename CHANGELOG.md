...
This changes what a re-run of `install codex-cli` does for an existing user who opts into nothing, and there is no switch that stops it adding the key. What keeps it out is a value of your own: a value you set yourself is left alone, so `mcp_optional_startup_grace_ms = 1000` on one line above the first table keeps Codex's own one-second grace through every later `install codex-cli`, `--force` and `--repair` included: install notes the value and leaves it.
