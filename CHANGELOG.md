...
Upgrading from 1.0.0, concretely. A local-scope install made with `--project-dir c:/repo` left two things behind: `projects["c:/repo"].mcpServers.mcp` in `~/.claude.json`, and `mcp__mcp__*` in `<repo>/.claude/settings.local.json`. On 1.0.0 which of the two the first `uninstall` cleared came down to how the drive letter was spelled the second time, and the trap is a mismatch rather than any one command. Repeat the install's own spelling -- `uninstall claude-code --scope local --project-dir c:/repo --force` -- and 1.0.0 did the whole job: `Removed the "mcp" entry.`, the settings patch, Done, and an empty container behind it. Uninstall the way the command is ordinarily run, though, with no `--project-dir` and the project as the working directory, and Git Bash and PowerShell both hand it `C:/repo`: 1.0.0 looked under `projects["C:/repo"]`, found nothing there, removed only the permission grant, printed "Claude Code no longer launches yaw-mcp" over an entry that still launched it for any session whose cwd carried the other spelling, and said "Nothing to do" on the second run. `--project-dir C:/repo` reached the same dead end. One run of this version clears both whichever spelling you type: the entry under whichever key holds it, and the grant.
