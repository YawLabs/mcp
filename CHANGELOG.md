...
Project and local scope are unchanged, since `CLAUDE_CONFIG_DIR` moves neither of their files, and so is every run outside a Yaw Mode pane. In a pane, the lines above replace the `install typed` note about a grant scoped to `CLAUDE_CONFIG_DIR`. `YAW_MODE` joins the `--help` Environment block.
