...
The check sits at the single call gate every path funnels through, which is what makes it cover a tool reached inside an `mcp_connect_exec` pipeline as well as one called directly. Its position there is the property, not a detail: it runs after every meta-tool branch, so the control surface the user manages the block with can never itself be blocked, and before the deferred-activation branch, which spawns the upstream with its vault-resolved credentials. One block later and a blocked call on a dormant server would start the process and inject the token before refusing.

A pipeline gets a second, earlier check. A step naming a blocked tool is refused before step one runs, because discovering it at step four costs the side effects of the three before it. That preflight is a convenience; the gate is the enforcement.

Blocked tools are withheld from the tool list, and every `discover` number that claims to describe that list moves with them -- the per-server count, the token estimate and the session total now run through one predicate, so none of them can drift from what the client actually receives. The ROUTE survives, deliberately: dropping it would turn a call by name into `Unknown tool`, which reads as a typo and sends the model hunting for a name that is right there. `discover` also annotates a blocked tool in the dormant list as `[blocked]` rather than dropping it, since that line describes what the SERVER offers and a silent omission reads as a broker bug rather than as the user's own policy.

The refusal is branded as a routing fault. Routes stay complete for a blocked tool, and `exec` books a persisted zero reliability score against the upstream for any unbranded step failure -- so without the brand, a user's own policy would have quietly degraded the health record of a perfectly good server.

Two loader warnings exist because the entry shapes are easy to get subtly wrong: an entry naming a meta-tool can never fire, and an entry with no `_` is almost always someone trying to block a whole server in the wrong key. A bare `*` is refused outright -- a deny that matches everything is far likelier a mistake than an intent.

**Changed -- `discover` stopped spending the context it exists to save**

For every server it had not connected, `discover` printed the complete list of cached tool names. On a 30-server install that was 20,536 of the reply's 22,712 bytes: 90% of a roughly 5,700-token answer, produced by the one meta-tool whose entire purpose is keeping tools out of context. The names are now capped at five per server with a `(+N more)` marker, and one short footer names the recovery path rather than repeating it on every line -- the repeated form would have cost 2,250 bytes where the marker plus footer costs about 450. A server at or below the cap renders exactly as before.

Two shapes benefit differently. Unranked, the body drops about 75%. Ranked -- `discover(context)` -- a server the query did not match at all now contributes no tool names either, since its card already says what it is and the names were pure noise; that takes the reply down about 87%, to roughly 725 tokens.

The recovery path is a new `server` argument: `discover(server: "tailscale")` returns just that server's card with its full, uncapped list and none of the cross-server blocks. Uncapped is safe there because it is bounded by one server and was explicitly asked for -- the worst real server measured, at 96 tools, renders about 750 tokens. There is deliberately no `limit` knob: its schema text would be a permanent per-session cost on every `tools/list` to expose a dial whose only correct value is the default, and a model that raised it would recreate the bug.

A focus miss reports which of the two states it hit, because they have different fixes: a disabled server needs a `bundles.json` edit, an absent one needs an install, and a near-miss name gets a did-you-mean. Focus never reaches the ranker, so `discover(context, server:)` can render one server's card while the banner reports that a different server was auto-loaded -- that banner announces a session change the model must know about, and hiding it would hide an activation.

**Fixed -- the session token total counted tools that were not in context**

The `N tools in context (~T tokens)` line accumulated its token half inside the per-server render loop, guarded only on a connection having tools, while the tool half required the connection to be `connected`. An advertised server that had errored therefore reported `0 tools in context (~1,234 tokens)`. The total is now computed once over the live connections, on the same terms as the count it sits beside: connected, advertised, after any tool filter. That also keeps it honest under the new focused render, which would otherwise have reported one server's cost as the session's.

**Added -- install says what to do next, and counts what it is leaving alone**

Install wrote the entry, said to restart the client, and stopped. It never read `bundles.json`, so a user with nothing configured was told to restart a broker with nothing to serve, and the natural next command went unnamed at the one moment they were still paying attention. It also read the client's sibling `mcpServers` only to detect its own legacy keys, so the servers the user already had were never mentioned -- leaving the reasonable impression that installing a broker moves them behind it.

It now ends with two lines about state it did not change. How many servers `bundles.json` will serve, or, when there are none, the `yaw-mcp add <slug>` step to take before restarting -- said in that order, because the restart line is the terminal instruction and advice printed after it is advice about a step already taken. And how many other servers sit in the client file, which keep launching directly. That second line is a COUNT, never the names: the same emitter feeds the `--dry-run` preview, which exists to be pasted into a bug report.

A `bundles.json` that is present and unusable is reported as a defect rather than as an empty file, with the loader's own warnings on stderr -- telling that user to `yaw-mcp add` would describe a broken file as an absent one. Both lines are read only after every refusal, so a malformed client config, an off-TTY collision or `--skip` prints neither; under `--all` the file is read once for the run rather than once per client.
