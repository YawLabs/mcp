...
Project scope is unchanged; the grant files of project and local scope are project-relative, so neither scope gets a second patch or a grant in the overlay. Claude Code's local-scope entry is the exception: it lives in `<CLAUDE_CONFIG_DIR>/.claude.json` under `projects[<dir>]`, so it is in the overlay too, and in a fresh pane it goes when the pane closes -- in this release without a note; the Unreleased section above adds one. Every run outside a Yaw Mode pane is unchanged as well. That includes a `CLAUDE_CONFIG_DIR` of your own set at a pane's prompt: Yaw exports `YAW_MODE` to the pane's shell too, but a directory not named `yaw-mode-*` is not an overlay, and install treats it exactly as it would outside Yaw. In a pane, the lines above replace the `install typed` note about a grant scoped to `CLAUDE_CONFIG_DIR`. `YAW_MODE` joins the `--help` Environment block.
