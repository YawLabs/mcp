...
The dependency range was `^1.8.0`, so every install resolved after 2026-09-22 got 1.9.0 -- `npx -y @yawlabs/mcp@latest`, a fresh `npm i -g`, and the copy Yaw Terminal 2.1.7 bundles -- while this repo's lockfile kept the suite on 1.8.0, where it passed. Run against 1.9.0, 170 of the 267 TOML and Codex tests fail. The check now accepts either plain-object prototype (arrays and `TomlDate` still fail it), the range moves to `^1.9.0` so the suite runs on the parser users get -- 1.9.0 also carries the fix for GHSA-r4xh-jqrq-34v2 -- and a test pins both prototypes directly instead of through whichever version the lockfile resolves. JSON and JSONC clients were never affected: their files go through jsonc-parser. Yaw Terminal users get the fix when the app bundles this release; until then `npx -y @yawlabs/mcp@latest install codex-cli` works once this version is published.
