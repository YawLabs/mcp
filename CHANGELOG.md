...
## Unreleased -- `install --force` stops keeping the env it promised to overwrite, editing a client config stops rewriting the entry next to ours, and the install surfaces stop stating things that are not so
...
- **augment** -- writes the grant to `~/.claude/settings.json` as well as the overlay's, and the `Wrote` line for `~/.claude/settings.json` ends ` -- a Yaw Mode pane's own settings.json does not outlive the pane`. `uninstall` removes it from both, and keeps each file's grant while another client that reads that file still has its entry. For `~/.claude/settings.json` that includes a Claude Code entry in the pane's own `.claude.json`, which Yaw may carry home, and the line kept for it says so: ``Keeping mcp__mcp__* in ~/.claude/settings.json: Claude Code (user) still launches yaw-mcp from <overlay>/.claude.json, which Yaw may carry home when the pane closes.`` May, not does: Yaw skips that copy when its `yawModeSyncClaudeJson` setting is off, and when `~/.claude.json` changed while the pane was open it merges instead, which brings no overlay `mcpServers` entry into a home file that already has `mcpServers`. The grant is kept for the case where the entry does arrive. An entry in any other file under the overlay -- typed's `<overlay>/.mcp.json`, say -- keeps only the overlay's grant, since that file goes with the pane.
...
Project scope is unchanged; the grant files of project and local scope are project-relative, so neither scope gets a second patch or a grant in the overlay. Claude Code's local-scope entry is the exception: it lives in `<CLAUDE_CONFIG_DIR>/.claude.json` under `projects[<dir>]`, so it is in the overlay too, and in a fresh pane `install claude-code --scope local` prints the note in its entry-only form: ``Note: a fresh Yaw Mode pane does not keep its config dir, so the Claude Code entry in <overlay>/.claude.json goes when this pane closes. Run `yaw-mcp install claude-code --scope local --project-dir <dir>` from a normal shell to keep it.`` Every run outside a Yaw Mode pane is unchanged as well. That includes a `CLAUDE_CONFIG_DIR` of your own set at a pane's prompt: Yaw exports `YAW_MODE` to the pane's shell too, but a directory not named `yaw-mode-*` is not an overlay, and install treats it exactly as it would outside Yaw. In a pane, the lines above replace the `install typed` note about a grant scoped to `CLAUDE_CONFIG_DIR`. `YAW_MODE` joins the `--help` Environment block.
