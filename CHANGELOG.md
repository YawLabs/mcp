...
A `config.toml` that really does not parse is reported in TOML words by `doctor`, `install` and `import`: ``exists but TOML is malformed -- install refuses to overwrite it; fix the TOML by hand, or move the file aside, then run `yaw-mcp install codex-cli` ``. A `mcp_servers` key that is not a table reads `not a TOML table` and `make it a table (or remove the key)`. An `mcp` entry written as an inline table, as dotted keys or as an array of tables is reported as present but not editable -- install will not rewrite it, and doctor cannot check its launch command -- and is not a warning.

`add`'s "no client is wired" nudge stops for a user whose only wired client is Codex CLI. `try` without `--client` still does not pick a `config.toml`: a trial marker cannot yet clean one up, so auto-detect keeps to JSON-family files. `doctor --json`'s `clients[]` slots gain two additive fields, `syntax` (`"JSON"` or `"TOML"`) and `entryUnspliceable` (the reason an `mcp` entry cannot be edited, or `null`).
