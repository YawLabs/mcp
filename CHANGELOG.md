...
None of that is in the published package. `files` is `dist`, `schemas`, `README.md` and `CHANGELOG.md`, so neither `scripts/lint.mjs` nor `release.sh` ships, and with `src/` untouched the shipped code is the 1.0.0 code.
