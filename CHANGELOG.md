...
**Fixed -- Codex CLI 0.151 and later get yaw-mcp's tools: `install codex-cli` sets `mcp_optional_startup_grace_ms = 0` at the top of `config.toml`**

Codex 0.156.1 waits 1000 ms for optional MCP servers while it builds a session's tool catalog -- the top-level `mcp_optional_startup_grace_ms`, whose default is 1000 -- and a server that misses that grace is left out for the whole session, not added on a later turn. yaw-mcp launched through the entry `install codex-cli` writes (`command = "npx"`, `args = ["-y", "@yawlabs/mcp@latest"]`) took from 3 to more than 35 seconds to start, so its tools were absent on all 8 turns of a 37-second session. With `mcp_optional_startup_grace_ms = 0` at the top of `config.toml`, Codex waits for each server's own `startup_timeout_sec` instead -- the entry sets 60 -- and the tools were present from the first turn. Both were measured on 2026-09-24 against real Codex 0.156.1, and Codex 0.144.0 accepts the key (no config error, and `codex mcp list` works). Codex's source has the key from 0.151.0 on; Codex 0.147 to 0.150 already wait a fixed 1 s that this key cannot change, so the fix needs Codex 0.151 or newer. Marking the entry `required = true` is not the fix: measured, a required server that has not started within `startup_timeout_sec` stops Codex with `required MCP servers failed to initialize ... timed out handshaking`, which would turn a slow or broken yaw-mcp into a Codex that does not start.

When the key is missing, `install codex-cli` now adds it as one line above the first table -- after the file's last top-level setting, or at the very top of a file that holds only tables, followed there by one blank line unless the file already opens on one -- in the same verified write as the entry, and prints `Added mcp_optional_startup_grace_ms = 0 to <file>: ...` once the write lands. A last line with no line break gets one, in the file's own line ending, so a CRLF file stays CRLF. For an existing setup, re-running `install codex-cli` over an entry that is already correct writes that one line (plus a blank line under it when the file starts with a table, as a file an earlier `install codex-cli` created does, and the final line break above when the file had none) and nothing else, where it printed `Nothing to do` before, and ends with `Restart it to pick up the change.`, not "the new MCP server"; the run after that is `Nothing to do` again. If a write that adds the line beside an entry that is already correct is refused, the error says the `mcp` entry is already correct and how to finish by hand: add the line above the first table in the file, and delete the legacy entry too when the same write was taking one out. A key already in the file is never changed, `--force` and `--repair` included: at 0 install says nothing about it, and at any other value (in any spelling, `"mcp_optional_startup_grace_ms" = 1000` included) it leaves the key and its value exactly as they are -- the entry is still written when it needs to be -- and says so once. For a value Codex takes, that is one `Note:` line naming the file, the value it found and why 0 is recommended, ending `Edit that value by hand to change it.` -- the value, not a line, since a multi-line string is one value over several lines. Codex's source types the key as a u64 at every release from 0.151.0 through 0.156.1, and measured on Codex 0.156.1, each of `-1`, `"0"`, `true`, `0.0`, `1979-05-27`, `[0]`, `{ a = 1 }` and `9223372036854775808` stops Codex at startup with exit 1 (Codex 0.144.0, which predates the key, loads all of them but the last). A value outside that type -- a float, a negative integer, a string, a boolean, a date or time, an array or a table -- gets a warning on stderr instead, `yaw-mcp install: warning -- <file> already sets mcp_optional_startup_grace_ms to ...`, which says what the value is and what Codex needs -- `0.0, a float where Codex CLI needs an integer`, `-1, a negative integer where Codex CLI needs a non-negative integer` -- and, in place of why 0 helps, `A Codex CLI release that reads the key will not load the file with that value; 0 is recommended.`, before the same last step. As with the warning about a typed CLI too old to read its file, the run still exits 0 and ends on its usual `Nothing to do` or `Done` line, and `install --all` counts the client as installed: whether the file loads depends on the Codex release. For an array or a table, which can be a `[mcp_optional_startup_grace_ms]` header and the keys under it, the last step is `To change it, delete it by hand and set the key on one line above the first table in the file.` instead, since a line written in place of a header that follows another table is a key of that table. A float such as `0.0` is another value, not 0, and the warning prints it as the file spells it. An integer past 9223372036854775807 is the exception to that exit 0, and so is one below -9223372036854775808: no TOML integer holds either, so no Codex release loads the file at all (measured for `9223372036854775808`, which 0.144.0 refuses at parse as 0.156.1 does), and install refuses to write into it, as it refuses a file its client loads nothing from -- `yaw-mcp install: <file> sets mcp_optional_startup_grace_ms to 9223372036854775808, larger than a TOML integer holds, so Codex CLI will not load the file -- refusing to write into it; change that value by hand to a non-negative integer no larger than 9223372036854775807 (0 is recommended), then re-run.` It exits 1, writes nothing and prints no `Nothing to do` or `Done` line, under `--dry-run` and `--skip` too, and `install --all` counts the client as failed. Only the key's own value is checked: an out-of-range integer inside an array or a table there gets the array-or-table warning, and one at any other key is not checked. `--skip` over an existing `mcp` entry leaves the whole file as it is, the key included; with no entry there is nothing to skip, and install writes the entry and the key as usual. `--dry-run` shows the line in its preview and prints `Would add mcp_optional_startup_grace_ms = 0 to <file>: ...`. The key follows `--scope project` (the project's `.codex/config.toml`) and `CODEX_HOME` exactly as the entry does. `uninstall codex-cli` leaves it in place -- it is not a server, and it only changes how long Codex waits for servers to start -- and when the file it edits still sets the key to 0, it says so in one `Keeping mcp_optional_startup_grace_ms at 0 in <file>: ...` line (`Would keep ...` under `--dry-run`), which names the key and its value rather than quoting a line -- the file may spell them `mcp_optional_startup_grace_ms=0`, `+0` or with a quoted key -- says the key stays because it is not a server and may be your own, and ends `Delete the line that sets it by hand if you no longer want it.`; a key at any other value, or none, gets no line. `doctor` and `install --list` only read the file -- they now say when the key is missing or refused, as an entry below describes -- and `heal`, `try` and `import`, which edit it, never add the key.

This changes what a re-run of `install codex-cli` does for an existing user who opts into nothing. For one run, `--skip` keeps the key out: over an existing `mcp` entry it leaves the whole file as it is, an entry that differs included. What keeps it out for good is a value of your own: a value you set yourself is left alone, so `mcp_optional_startup_grace_ms = 1000` on one line above the first table keeps Codex's own one-second grace through every later `install codex-cli`, `--force` and `--repair` included: install notes the value and leaves it.

**Fixed -- `doctor` and `install --list` say when a Codex CLI `config.toml` lacks `mcp_optional_startup_grace_ms`, or sets it to a value Codex will not load**

Both read a Codex setup from before `install codex-cli` added the key as healthy -- `OK -- has "mcp" entry` and `installed` -- though it is the setup whose tools Codex 0.151 and later leave out whenever yaw-mcp takes more than a second to start (the entry above). Each now asks the plan `install` makes for the same file, so the three cannot disagree about which key a file lacks or which value Codex refuses. Beside yaw-mcp's `mcp` entry:

- **The key is missing.** doctor's CLIENTS line keeps its `OK` and ends ``; the file does not set mcp_optional_startup_grace_ms -- run `yaw-mcp install codex-cli` to add mcp_optional_startup_grace_ms = 0`` -- or, on a line that already says to rerun install, `-- install adds mcp_optional_startup_grace_ms = 0 as it writes the working entry`, and beside an entry whose launch path is for another OS, `-- set mcp_optional_startup_grace_ms = 0 by hand, on one line above the first table in the file`. DIAGNOSIS gains ``Run `yaw-mcp install codex-cli` to add mcp_optional_startup_grace_ms = 0 to <file>: <why>.`` under whichever verdict it prints, once per file, and not for the other-OS entry. It is not a warning, and the exit code stays 0: the file loads and yaw-mcp starts, the precedent a legacy entry beside a working one set. `install --list` shows `installed (setting missing)`, with a note under the table: ``Note: <file> has the Codex CLI (user) entry but does not set mcp_optional_startup_grace_ms = 0: <why>. `yaw-mcp install codex-cli` adds it.`` For the project row the run it names carries `--scope project`, and `--project-dir` when `--list` was given one.
- **The value is one a Codex that reads the key will not load** -- `-1`, `"0"`, `0.0`, `true`, a date, an array, a table. doctor's line drops its `OK -- ` and ends `; the file sets mcp_optional_startup_grace_ms to -1, a negative integer where Codex CLI needs a non-negative integer -- a Codex CLI release that reads the key will not load the file with that value, and install leaves a value you set alone; edit that value by hand (0 is recommended)`. It is a warning, so doctor exits 2, as it does for our entry in a file its client cannot load for its syntax. `install --list` shows `installed (setting refused)`, with a note in the words of install's own warning.
- **The value is one no Codex release loads** -- an integer past 9223372036854775807 or below -9223372036854775808. doctor's line is ``exists but sets mcp_optional_startup_grace_ms to 9223372036854775808, larger than a TOML integer holds, so Codex CLI will not load the file -- install refuses to write into it; change that value by hand to a non-negative integer no larger than 9223372036854775807 (0 is recommended), then run `yaw-mcp install codex-cli` `` in place of the row's usual wording -- which, for a file with no entry of ours, was ``present, no "mcp" entry -- run `yaw-mcp install codex-cli` ``, a run install refuses -- and a warning when our entry is there. `install --list` shows `installed (setting refused)`.

A value Codex takes, such as `1000`, is yours, and neither command says anything about it. `doctor --json` ends `.diagnosis.summary` with the DIAGNOSIS step, word for word, and every `clients[]` item gains `rootDefaults`: one item per top-level setting the file does not hold at the value install writes, with `key`, `recommended`, `why`, `state` (`missing`, `other`, `refused` or `refused-every-release`), `value` (always a string), `kind`, `needs`, `everyRelease` and `fix`, each null in a state that has none. It is `[]` on a JSON-family row, on a slot with no file, and on a file doctor cannot read or install will not edit. No existing field changed. `install --list` still counts these rows as installed in its headline.

**Fixed -- `install --help` no longer calls a re-run over a matching Codex CLI entry a no-op**

`install --help` said that re-running install over an entry that already matches is a no-op, and that `--repair` is one on such an entry. For Codex CLI both stopped being true when install began adding `mcp_optional_startup_grace_ms = 0` (the first entry above). The text now reads:
...
```
  ...
  prompt) -- unless the file lacks a top-level setting its client needs:
    Codex CLI: mcp_optional_startup_grace_ms = 0
...
The indented line is built from the client table rather than written by hand. `--repair` over a matching entry `does what a re-run does (above)`, `--skip` leaves `the existing entry, and the rest of the file, untouched`, and `--dry-run` lists a top-level setting it would add with the entry and any permissions patch. A new paragraph says that a file install will not write into -- one that does not parse, for example, or one that sets such a setting to a value no release of its client loads -- exits 1 with nothing written, under `--dry-run` and `--skip` too. In `yaw-mcp --help`, `install <client>` says that for codex-cli it also adds the key, even on a re-run over an entry that is already correct, and the `install --list` legend names `(setting missing)` and `(setting refused)`.

**Fixed -- `install`'s Done line names a new MCP server only when the run wrote one**

A re-run of `install` over an entry that was already correct, whose only write was taking a legacy key (`mcp.hosting`, `mcph` or `yaw-mcp`) out of the file or adding yaw-mcp's pattern to Claude Code's `permissions.allow` (for Claude Code and typed), still ended with `Restart it to pick up the new MCP server.` -- or the row's own form of it, such as `Cline starts the server when the file is saved -- no restart needed.` or `typed picks the entry up in its next session.` -- for a server the run had not added. Those runs now name the change, in the row's own words: `Restart it to pick up the change.`, `Cline picks the change up when the file is saved -- no restart needed.`, `typed picks the change up in its next session.`, `Reload the IDE window to pick up the change.` The same goes for a legacy key taken out of one of Cline's editor copies when every entry was already correct. A run that adds or changes the `mcp` entry in any file it writes -- the client's own file or an editor copy -- prints the Done line it printed before, byte for byte, with or without a legacy trim. `install --all` prints each client's Done line from the same code, so the same holds there. Claude Code at `--scope project` keeps its own Done line, which asks you to approve the `.mcp.json` server when prompted, on every run that writes a file: install cannot see whether it was approved. A run with nothing to write prints `Nothing to do` instead, as it did before.

**Fixed -- a CRLF config file with no final line break stays CRLF when yaw-mcp writes it**

When a client config it rewrites had no line break at the end, yaw-mcp added one -- in `install`, `uninstall`, `try` and `try-cleanup`, `doctor` when it sweeps an expired trial out of a client config, and `import` when it takes the originals out -- and that line break was always a bare LF. What yaw-mcp splices in already copies the file's own line ending, so a CRLF file with no final line break came back CRLF on every line but the last. Reproduced with `install codex-cli --repair` (or `--force`) replacing the `[mcp_servers.mcp]` table in the middle of such a `config.toml`; the same happened to a CRLF `~/.claude.json`, `.mcp.json` or Cline settings file, editor copies included, and to Claude Code's `settings.json` when `install` or `uninstall` changed its `permissions.allow`. The final line break now uses the file's own line ending, the one its first line break uses (CRLF, LF or a lone CR), and LF when the file has no line break at all. A file that already ends in a line break keeps exactly the one it had. A lone CR at the very end is that line break only in a file whose first line break is a lone CR; anywhere else an LF goes after it, which completes a CRLF file's last line break and gives an LF file the final LF it always got, so an LF file is written as before. A `config.toml` that holds nothing but whitespace is still written as a new file, as a missing one is; when that whitespace's first line break is a CRLF, the new file is now CRLF too, where it was LF.

**Fixed -- `heal` ends a client config it rewrites with a line break**

When `heal` -- the `yaw-mcp heal` command, and the same sweep `serve` runs at startup -- re-pointed a stale entry in a client config whose last line had no line break, it wrote the file back still without one, where `install`, `uninstall`, `try` and `import` add one to a file they rewrite. (A `config.toml` whose `[mcp_servers.mcp]` table came last was the exception: replacing that table rewrote the file's end, line break included.) It now adds one too, in the file's own line ending as the entry above describes, and a file that already ends in a line break keeps exactly the one it had. A run that re-points nothing, and `heal --dry-run`, still write nothing. The startup pass makes this change with no command run, and `YAW_MCP_AUTO_HEAL=0` turns that pass off, as before; `yaw-mcp heal` run by hand ignores the variable.

**Fixed -- a client config `install` or `uninstall` cannot write is named, with the step past it**

A failed write printed node's error, which named the temp file the write renames from and said nothing to do. Measured on Windows 11 with Node 22: a read-only `config.toml` gave `failed to write <file>: EPERM: operation not permitted, rename '<file>.tmp-...' -> '<file>'`, and a `CODEX_HOME` pointing at a file gave `EEXIST: file already exists, mkdir '<CODEX_HOME>'`. The message now names the file you know and the step past it:

- A read-only file: ``yaw-mcp install: failed to write <file>: it is read-only -- clear its read-only attribute (`attrib -R "<file>"`), then re-run.`` On macOS and Linux a read-only file does not stop the write, which replaces the file through its directory.
- A file where a directory has to be: `failed to write <file>: <that file> is a file, not a directory -- move or rename it, then re-run`, or, when an environment variable put the config under it, `failed to write <file>: <that file> is a file, not a directory, and CODEX_HOME (set to <value>) puts config.toml under it -- point CODEX_HOME at a directory, then re-run`. On macOS and Linux that fault stops the read before any write, and the `cannot read <file>:` refusal of `install` and `uninstall` now ends the same way, where it ended on node's `ENOTDIR: not a directory`; `try` gets the clause without the variable.
- On Windows, a file another program holds: `another program may be holding it open, or your account may not replace it (EPERM) -- close whatever has it open (an editor, a sync client, a virus scanner) or fix its permissions, then re-run`. A directory that will not take a new file: `your account may not create or replace files in <directory> (EACCES) -- fix that directory's permissions, then re-run`.

Any other error keeps node's message, plus the variable to check when the directory it names could not be made. Both commands still exit 1 with nothing written. `heal` words a failed write the same way (the next entry). The writes of Claude Code's `settings.json` permissions and of Cline's per-editor copies keep their wording, which already gives a by-hand step.

**Fixed -- `heal` reports a stale entry it could not re-point, and exits 1**

When `heal` found a stale entry and its rewrite failed -- a read-only `config.toml` on Windows, say -- the failure went to a log line and nowhere else: `yaw-mcp heal` printed `No stale yaw-mcp entries found.` and exited 0, straight after `heal --dry-run` had said `Would re-point 1 stale entry`. Such an entry now goes to stderr in every mode, `--json` and `--quiet` included, and the run exits 1:

```
yaw-mcp heal: could not re-point 1 stale entry:
  codex-cli (user): <file>
    still -> <launch file that no longer exists>
...
The last line of each entry is the write-failure wording of the entry above, or, for an edit refused before any write (a file its client cannot load, a splice that will not verify), `failed to splice the "mcp" entry into <file> (<why>)`. `heal --dry-run` now asks that same splice, so it says `cannot re-point` for an entry the live run would refuse, where it said `Would re-point`. `No stale yaw-mcp entries found.` prints only when the sweep found no stale entry, re-pointed or not. `heal --json` gains `failed`, one item per such entry with `clientId`, `scope`, `path`, `from`, `to` and `error`; `healed`, `unhealable`, `count` (still repairs only) and `dryRun` are unchanged. The startup pass `serve` runs still logs one `Could not heal a stale yaw-mcp entry` warning per such entry and carries on, now with that clause in place of node's error; `YAW_MCP_AUTO_HEAL=0` turns that pass off.

**Fixed -- `uninstall` over an `mcp` entry it will not edit says so instead of calling it absent, and `install` names the fix**

Codex reads an `mcp` entry written as an inline table under `[mcp_servers]`, as dotted keys, or inside an inline `mcp_servers = { mcp = { ... } }`, and will not load a file that writes it as an array of tables; yaw-mcp's TOML editor rewrites none of these shapes. `uninstall codex-cli` read such a file as holding no entry and printed `Nothing to do: Codex CLI (user) has no yaw-mcp entry.` at exit 0, while `doctor` and `install --list` reported the entry and, in every shape but the array of tables, Codex went on launching it. It now refuses, exit 1 with nothing written, before its confirmation prompt and under `--dry-run` too: `yaw-mcp uninstall: the "mcp" entry in <file> is <shape>, which uninstall will not edit -- nothing was removed; <fix>, then re-run.` `install` exited 1 over the same file before and still does, under every flag, but ended on `failed to splice the "mcp" entry into <file> (...). Refusing to overwrite.` with no step, after a `Runtime:` line for an entry it never wrote. It now refuses before that line: `yaw-mcp install: the "mcp" entry in <file> is <shape>, which install will not edit -- refusing to overwrite it; <fix>, then re-run.` `<shape>` and `<fix>` are the TOML reader's own, the pair doctor's CLIENTS line prints for the file -- for the inline container, `inside the inline table mcp_servers = { ... }` and `convert the inline mcp_servers = { ... } to [mcp_servers.mcp]-style tables by hand`.

**Fixed -- a `config.toml` that is not valid UTF-8 is refused, not written back with replacement characters, and `doctor` calls it malformed**

yaw-mcp decoded a client config as UTF-8 and turned each byte sequence that is not UTF-8 into U+FFFD. `install codex-cli` over a `config.toml` holding such bytes -- `model = "o3<FF><FE>"`, say -- wrote the U+FFFD characters back in their place and exited 0, changing a value that was not yaw-mcp's in a file Codex does not load: measured on Codex 0.144.0, `codex mcp list` over that file exits 1 with `Failed to read config file <path>: invalid utf-8 sequence of 1 bytes from index 11`. A TOML config that is not well-formed UTF-8 is now refused as malformed, with the 0-based byte offset of the first bad sequence, the index Codex reports: `yaw-mcp install: <file> is not valid TOML (invalid UTF-8 at byte offset 11, and a TOML file must be UTF-8 -- re-save the file as UTF-8) -- refusing to overwrite it; fix the TOML by hand, or move the file aside, then re-run.`, exit 1 and nothing written. `uninstall`, `try`, `try-cleanup`, `import` and `heal` read through the same code: `uninstall`, `try` and `import` refuse the file with the same detail, `try-cleanup` keeps the trial marker and names the offset in its reason, and `heal` lists the file under `could not be checked`.

`doctor` and `install --list` read the same bytes the same way. They decoded the file on their own and classified the U+FFFD text, which parses, so doctor reported such a file as ``present, no "mcp" entry -- run `yaw-mcp install codex-cli` `` -- a run that now refuses it -- or as `OK` when our entry was in it, and `heal`'s ``Run `yaw-mcp doctor` for what each one needs.`` led to a report that found nothing wrong. For that file doctor's CLIENTS line is now ``exists but TOML is malformed (invalid UTF-8 at byte offset 11, and a TOML file must be UTF-8 -- re-save the file as UTF-8) -- install refuses to overwrite it; fix the TOML by hand, or move the file aside, then run `yaw-mcp install codex-cli` ``, a warning like any malformed client config, so doctor exits 2, and `install --list` shows the row as `malformed`. `doctor --json` gains `malformedDetail` on every `clients[]` item: that detail on such a file, and null everywhere else -- a file that does not parse still gets no detail, and its line reads as it did.

A BOM, CRLF line endings and a real U+FFFD in the file read as before. JSON client configs are still decoded as they were.

**Fixed -- `try-cleanup` and doctor's expired-trial sweep take a Codex CLI trial out of `config.toml`, and `try-cleanup` no longer says `cleaned up` over an entry it left**

Both read the client config a trial was written into as JSON, whatever the client, so neither could remove a `yaw-mcp try <slug> --client codex-cli` trial. `try-cleanup` failed to parse `config.toml`, printed `warning -- couldn't strip ...`, deleted the trial marker anyway and ended `cleaned up` at exit 0, leaving the trial's entry -- with any `--env` value written inline in it -- wired into Codex and nothing on disk naming it, and `doctor` saying all was well. Doctor's sweep of an expired Codex trial failed the same way on every run, with ``trial "<slug>": expired but could not be removed from <file> (...) -- still wired in; run `yaw-mcp try-cleanup <slug>` or edit that file by hand`` and exit 2. Each now reads the file in the syntax the trial was written in. A trial marker records it in a new `format` field (`toml` for Codex CLI, `jsonc` for the JSON-family clients); a marker written before this release has none and is read as TOML when its file name ends in `.toml`, JSONC otherwise; and a marker naming a format this build cannot read is refused, as one from a newer schema is. An older yaw-mcp ignores the field, so it still cannot remove a Codex trial.

`try-cleanup` also stops when the entry cannot be taken out for any other reason -- a file that does not parse or cannot be read, an entry in a form yaw-mcp will not edit, a file that is not a table or an object -- instead of warning and carrying on. It prints `yaw-mcp try-cleanup: couldn't strip <entry> from <file> (<reason>) -- it may still be wired in, so the trial marker was kept.` and, indented under it, `Fix that file (or remove the <entry> entry from it by hand), then re-run: yaw-mcp try-cleanup <slug>`, keeps the marker, and exits 1, where it exited 0 -- what doctor's sweep already did with the same failure. A cleanup that succeeds prints what it did before.

**Changed -- `import` asks before it changes a server bundles.json already has, and off a terminal it needs `--force`**

An imported server that matches an entry already in `~/.yaw-mcp/bundles.json` is merged into it with the client's copy winning, and an imported entry carries no catalog slug, so nothing refused the merge: `import` named the entry in its plan (`REPLACES the "<namespace>" entry already in bundles.json:`, with the launch it replaced) and overwrote it without asking, off a terminal too. A changed launch command there is a different program run as you on the next activate. When the merge would change the stored entry, `import` now asks first, before anything is written:

```
<bundles.json> already has an entry "fs" that differs from the one importing fs would write:
...
The diff lines are install's own for a differing `mcp` entry: `command` and `args` with their values, `env` by key only, anything else by field name. `[o]verwrite` prints `Overwriting the "fs" entry in bundles.json.` `[s]kip`, or a bare Enter, leaves the entry as it is and does not import that server -- `Left the "fs" entry in bundles.json as it is -- fs is not imported.` -- so its original stays in the client config, `--remove-originals` included. `[a]bort` exits 1 and Ctrl+C exits 130, each with nothing written, the servers that collide with nothing included. A run in which every server was skipped prints `Nothing imported: every server was skipped, so bundles.json and <file> are unchanged.` and exits 0. A stored entry the merge would leave as it is -- key order aside -- is not asked about.

Off a terminal, where it overwrote before, `import` now writes nothing and exits 2, the code `install` uses for the same refusal: `yaw-mcp import: <bundles.json> already has an entry this import would change, and there is no terminal to ask on. Nothing was written.`, then each entry's diff, then `Re-run with --force to overwrite it, or --dry-run to preview.` `--force` is the new flag that keeps the old non-interactive behaviour: it overwrites every entry the import would change without asking, and prints `Overwriting the "<namespace>" entry in bundles.json (--force):` with the diff. `--dry-run` asks nothing and prints `Would overwrite the "<namespace>" entry in bundles.json (a real run asks first, and off a terminal needs --force):` with the diff. `import --help` describes the question and the flag.

**Fixed -- `import --dry-run --remove-originals` lists the client entries a real run would remove**

The dry run said nothing about the originals, so the one step of an import that edits a file you did not name could not be previewed. With `--remove-originals` it now prints, before `--- dry run: nothing was written ---`, `Would remove 2 entries from <file> (--remove-originals):` and one indented line per key -- after `Reached through the yaw-mcp entry in <container>.` when yaw-mcp is reached through another scope's entry -- or the reason it would remove none: the real run's refusal, led by `Would not remove the originals: ` in place of `Not removing the originals: `; `yaw-mcp import: would not be removed from <file>: <key> (<why>). Remove that entry by hand; ...` for a key the real run would leave; `yaw-mcp import: would remove nothing from <file>; it would be left unchanged. The servers would be imported either way.`; or, when bundles.json does not parse and nothing would be imported, `yaw-mcp import: would remove nothing from <file> -- none of these servers would be imported, because their write to bundles.json could not be previewed (the warning above says why).` The preview runs the real run's own search for yaw-mcp's entry, its refusal and its splice over the same bytes, and the real run's removal prints what it did before. The preview assumes that every bundles.json entry the import would change is overwritten (the entry above); a real run in which one is skipped leaves that server's original where it is. A dry run without `--remove-originals` prints nothing new, and `import --help` says what `--dry-run` shows with it.

**Changed -- the oam floor moves to 0.17.0**

`npm run verify:oam-floor` hosted a stdio `@modelcontextprotocol/sdk` server on oam v0.17.0 through `oam run` on 2026-09-25 -- initialize, tools/list and tools/call all completed -- and raised `MIN_OAM_VERSION` to it; the floor was 0.16.4. A machine whose oam is older hosts its node/npx sidecars on node instead, and logs a warning naming both versions, `oam self-update` as the fix, and that yaw-mcp needs a restart afterwards.

## 1.0.15 -- the missing-credential prompt catches more servers and says what is true, and `yaw-mcp secrets --json` keeps stderr to one JSON object per line

Published 2026-09-24, the same day as 1.0.13. This section was prepared under a 1.0.14 heading and published as 1.0.15: there is no 1.0.14 on npm and no `v1.0.14` tag. Sixteen entries below, from a full pass over the secrets and missing-credential code (#173). The credential prompt recognises more of the ways a server says a key is missing, no longer asks for a key bundles.json already sets, drops a value the server rejects, and its refusals say what the next activate will do. `yaw-mcp secrets --json` keeps stderr to one JSON object per line, and its prose refusals name their action. Six of the changes alter behaviour for an existing setup that opts into nothing, and none sits behind a switch: `secrets list --json` and `secrets audit --json` print one line; an inherited value of a name ending in `_PATH`, `_FILE`, `_DIR`, `_HOST`, `_PORT`, `_PREFIX` or `_SIZE` is no longer masked in a child's stderr tail or prompted for; more stderr phrasings bring up the credential prompt; a non-empty bundles.json value wins over a typed one; a vault with an array `entries` or scrypt-invalid kdf parameters is refused at load; and a Tab pasted at the `secrets set` value prompt is kept.
...
1.0.13 moved the missing-credential prompt onto the masked page (#170). Three things it did or said were wrong:

- An expired page returned the plain `Failed to load "<server>": ...` error, which says nothing about the page, and was booked as a failed activation, which is what `discover`'s failure warning and `dispatch`'s down-ranking read. It now fails with `Could not load "<server>": the page that takes the credential in a masked field expired with nothing submitted, so GITHUB_TOKEN is still missing.` and is not booked: the server's only problem is a value nobody got to type. While the server has an ask left, the message goes on `Activate "<server>" again for a new page, or set GITHUB_TOKEN in its "env" in the bundles.json that defines it (...) -- the edit is picked up on the next mcp_connect_* call, with no client restart.`; when the expired ask was its last, it says no further prompts are coming, as below. An expired page still does not end the asking.
- A page that could not start, or a browser that could not be opened for it, ends the asking for that server, yet its refusal said `Activate "<server>" again for a new page`, and every later activate repeated that without opening one. It now says `No further prompts for "<server>" this session: set GITHUB_TOKEN in its "env" in the bundles.json that defines it (...) and activate again -- the edit is picked up on the next mcp_connect_* call, with no client restart.`, the same line that ends the refusal for values the server did not accept once the asks are spent.
- With more than one key missing, these refusals say `credentials` and `are still missing` (1.0.13's two said `credential` and `is still missing`), and the page's intro says the server `reported its missing keys on its own`, not `their missing keys on their own`.

**Changed -- a credential typed at the prompt fills a gap in bundles.json instead of overriding it**

A value typed at the missing-credential prompt was merged over the server's `env` from bundles.json, so for the rest of the session it beat whatever the file said. The fix the prompt's refusals recommend -- set the key in bundles.json and activate again -- then did nothing until a client restart, and the reload that picks up a bundles.json edit saw no change to the server's launch either, so it left the server running on the typed value. A typed value now fills only a key the server's entry leaves unset or empty (a `"GITHUB_TOKEN": ""` placeholder counts as unset); a non-empty value in the file wins. A file value that differs from the typed one changes the server's launch config like any other `env` edit, so the reload unloads the running server (once no call is in flight on it) and the next activate starts it with the file's value. A key the entry already sets to a non-empty value is not asked for at all, since the answer would be ignored; when every key the server names is set there, the activation returns the plain `Failed to load` error. `mcp_connect_read_tool`'s one-off connect, which uses the typed values so that inspecting a schema does not trip the same missing-credential error, follows the same rule.

**Fixed -- a credential the server rejects is dropped instead of riding along on later launches**

When the retry a prompt bought fails with the server still naming the same keys missing, the activation says the values just provided were not accepted. The rejected values stayed in memory anyway, and went into every later launch of that server for the rest of the session until a new ask replaced them. They are now dropped at that point. A value the server did not name again -- one it accepted -- is kept.

**Fixed -- the missing-credential prompt recognises more of the ways a server says a key is missing, and a line no longer runs into the next**

The stderr patterns that bring up the prompt now also match:

- `environment variable` or `env var` between the name and the verb: `GITHUB_TOKEN environment variable is required`, `The GITHUB_TOKEN env var is not set`, `OPENAI_API_KEY environment variable must be set`.
- The plural: `Missing environment variables: OPENAI_API_KEY`, `Missing env vars: GITHUB_TOKEN`.
- A name in backticks or quotes: `` `GITHUB_TOKEN` is required ``, `Missing env var 'GITHUB_TOKEN'`, `"GITHUB_TOKEN" is not set`.
- `not set`, `unset` and `not provided` without `is`: `GITHUB_TOKEN not set`, `env var SLACK_BOT_TOKEN not set`. `is` is still required before `required`, `missing`, `empty` and `undefined`, where without it the word describes something else (`GITHUB_TOKEN required scopes` is not a match), and `is not defined` -- the JavaScript `ReferenceError` shape -- still is not a match either.
- `No GITHUB_TOKEN provided` and `No GITHUB_TOKEN set`.
- `Please set` with `the`, `environment variable` or `env var` in front of the name, in the plural, or with a colon: `Please set environment variable GITHUB_TOKEN`, `Please set the environment variable GITHUB_TOKEN`, `Please set the GITHUB_TOKEN environment variable`, `Please set env var: GITHUB_TOKEN`, `Please set environment variables: GITHUB_TOKEN`.

The gaps inside a pattern no longer cross a line break, except after a colon. The text searched is the stderr tail and the error message joined with newlines, so a line ending in `missing` (`sourcemap is missing`) could claim a credential-shaped name at the start of the next line (`OPENAI_API_KEY loaded from vault`) and put up a prompt for a key the server never asked for. `Missing:` with the name on the next line still matches, as a list. Only the first name of a comma-separated list is asked for (`Missing required environment variables: OPENAI_API_KEY, GITHUB_TOKEN` asks for `OPENAI_API_KEY`); a server that then names the second on the retry gets an ask for it while it has one left. A warning line such as `GITHUB_TOKEN not set, using unauthenticated` now matches too; the patterns are only read once a start has failed.

**Fixed -- a name ending in `_PATH`, `_FILE`, `_HOST` and the like is no longer taken for a credential**

The classifier that decides which environment variable names read as credentials counted a name as one when any segment of it did, so `SSH_KEY_PATH`, `TLS_KEY_FILE`, `AWS_SHARED_CREDENTIALS_FILE` and `TOKEN_BUCKET_SIZE` all counted. A name whose last underscore-separated segment is `PATH`, `FILE`, `DIR`, `HOST`, `PORT`, `PREFIX` or `SIZE` now does not: it names where a secret lives, or a property of something, rather than the secret. Two things read that classifier. The missing-credential prompt no longer opens a masked secret field for a file path. And the redaction of a failed start's stderr tail no longer treats an inherited environment variable with such a name as a secret: its value -- a path, say -- was replaced with `***NAME***` wherever it appeared in that output. Values yaw-mcp itself injected from bundles.json or the vault are still masked whatever their names. Names ending in `_URL`, `_URI` or `_ID` are unchanged -- a connection string can carry its password inline (`REDIS_PASSWORD_URL`), and `AWS_ACCESS_KEY_ID` is half of a credential pair -- and a name that ends in a credential segment, such as `PATH_TOKEN` or `HOST_KEY`, still counts.

**Fixed -- messages that say to edit bundles.json name the project-local file too**

A trusted project-local `.yaw-mcp/bundles.json` replaces the user-global file outright while it is in effect, with no merge, so an edit sent to `~/.yaw-mcp/bundles.json` can land in a file the session never reads. Several messages named only the global file: `discover`'s focused result for a disabled server or for a name no entry defines, `mcp_connect_read_tool`'s refusal of such a name, the refusal to load a disabled server, the missing-credential prompt and its refusals, and the pointer every activation and connect failure ends with -- including the raw error returned when a credential prompt is declined or its asks are spent, which read `-> Fix in ~/.yaw-mcp/bundles.json under "<server>", then activate it again ...` and now reads `-> Fix "<server>" in the bundles.json that defines it (...), then activate it again ...`. They now say "the bundles.json that defines it (~/.yaw-mcp/bundles.json, or a trusted project-local .yaw-mcp/bundles.json)", or "the bundles.json in effect (...)" for a name no entry defines -- the wording the no-servers-enabled message already used. The two messages about `yaw-mcp add` still name `~/.yaw-mcp/bundles.json` alone, because that is the file it writes.

**Fixed -- a URL-mode client is told when a masked-entry page stops on its own**

For a client that opens the masked-entry page from a URL-mode prompt, the broker sends `notifications/elicitation/complete` when the page is done, so the client can drop whatever waiting state it shows. It was sent when the page took a value or expired, and skipped when the page closed, on the reasoning that only shutdown closes a page. The page also closes when its listener fails after it has started listening, and that client was left waiting. The notification now goes out however the page ended, for the vault passphrase and the missing-credential page alike, and is skipped only once shutdown has begun.

**Fixed -- the Windows `taskkill` that stops a timed-out child no longer inherits yaw-mcp's own secrets**

yaw-mcp removes its own secrets (`YAW_MCP_VAULT_PASSPHRASE`, `YAW_MCP_VAULT_PASSPHRASE_NEW` and the retired `YAW_MCP_TOKEN`, in any letter case) from the environment of the processes it starts. Two did not get that: the `taskkill /T /F` that `yaw-mcp compliance` runs on Windows when the compliance suite times out or overruns its output limit, and the one that stops an `npm prefix -g` probe that has run past 3 seconds (the probe `yaw-mcp upgrade`, `doctor` and the background self-upgrade run to find npm's global prefix). Both inherited the whole environment; both now get the stripped one. A source scan in the test suite checks every spawn-family call in a file that imports `node:child_process` for the strip.

**Fixed -- `secrets rotate` says an entry is corrupt, not that the passphrase is wrong, when the vault has just verified the passphrase**

`rotate` decrypts every entry into memory before it re-encrypts anything, and aborts with the vault untouched if one fails. That failure read `rotate aborted: entry "X" failed to decrypt under the current passphrase` even on a vault whose check marker had just verified the passphrase, so it blamed the passphrase for a damaged entry -- and `secrets reset`, the way out of a wrong passphrase, refuses one that opens the vault. On a vault with a marker the message now says the passphrase is right and the entry is corrupt (or was written under a different passphrase than the rest of the vault), and to remove it with `yaw-mcp secrets remove X`, set it again, and re-run `rotate`. A vault with no marker, where nothing has verified the passphrase, keeps the old message.

**Fixed -- `doctor` and `secrets reset` no longer read a failed key derivation as a wrong passphrase**

`checkVaultPassphrase`, the check behind doctor's SECRET VAULT passphrase line and `secrets reset`'s refusal to reset a vault the passphrase already opens, answered "wrong" for any failure to unlock -- including a failure of the scrypt key derivation itself, which says nothing about the passphrase (in practice, a host that cannot allocate the working set). doctor then printed that the passphrase does NOT unlock the vault, with its pointer at `secrets reset`, and `reset` carried on as though the passphrase were wrong. The check now answers "wrong" only when nothing in the vault decrypts, or for an empty passphrase, and passes any other failure up. doctor prints the plain `passphrase: set in this environment` line for it (`vault.passphraseUnlocks` is `null` under `--json`), and `reset` refuses, exit 1, before anything is copied or written: ``could not check YAW_MCP_VAULT_PASSPHRASE against the vault at <path> (<cause>) -- nothing was changed.`` (`that passphrase` in place of the variable's name for one typed at the prompt; one `{"ok":false}` envelope under `--json`).

**Fixed -- two malformed vault files are refused as corrupt when they are loaded, like every other**

A vault whose recorded KDF parameters pair `r=1` with `N` of 2^16 or more passed the load check, although OpenSSL's scrypt refuses that pair outright (it requires `N < 2^(16r)`), and then failed at unlock with node's own `ERR_CRYPTO_INVALID_SCRYPT_PARAMS` error -- which the broker reported as a passphrase that does not unlock the vault. It is now refused at load as `vault at <path> is corrupt: invalid kdf parameters`, as every other bad pair is. And a vault whose `entries` is a JSON array is refused as `missing or invalid salt/entries`: `[]` used to load as an empty vault, and `["x"]` failed naming a corrupt entry `0` that is not there. This build writes neither shape.

**Changed -- the two messages about a damaged verification marker say what is true**

The error for a vault whose `check` marker is damaged, which every command that unlocks the vault gives, now ends `the check marker does not verify` rather than `does not decrypt`: it is also raised when the marker decrypts to the wrong text. And the warning for a marker too malformed to use, which is then ignored, now says the next `yaw-mcp secrets set` re-stamps it rather than "the next write" -- a `remove` leaves the vault without one.

**Fixed -- `yaw-mcp secrets --json` keeps stderr to one JSON object per line in the three places it did not**

`secrets --help` promises that under `--json` stderr carries one JSON object per line. Three things broke that:

- An argument error printed prose followed by the whole usage text (`yaw-mcp secrets: unknown flag "--bogus"` and so on). With `--json` anywhere in argv it is now the one line `{"ok":false,"error":"unknown flag \"--bogus\""}`, still exit 2. `--help` still prints the usage.
- `get`'s warning that it is printing a value in cleartext to a terminal was prose. It is `{"warning":"cleartext-on-tty","name":...}` now.
- A malformed check marker was reported by the vault loader's own `yaw-mcp: warning: ...` log line, on a command that then succeeded, and a vault file that is not JSON or cannot be read put the same kind of line ahead of the `{"ok":false}` envelope. `list`, `get`, `set`, `remove`, `rotate` and `reset` now print the marker warning themselves: `{"warning":"check-malformed","path":...,"message":...}` under `--json`, `yaw-mcp secrets: warning -- <path>: <message>.` otherwise. Where a vault that is not JSON or cannot be read is refused, the refusal is now the only line, with or without `--json`. For a file that cannot be read, that line now names it: `list` and `get` print `could not read the vault file at <path> (<errno>) -- fix that and re-run.`, the sentence `set`, `remove`, `rotate` and `reset` already used, where before they printed Node's bare errno message, which for `EISDIR` or `EIO` names no file.

The help text lists the new warning kinds and says argument errors are covered.

**Changed -- `secrets list --json` and `secrets audit --json` print their result on one line**

Both pretty-printed their stdout envelope over several lines, where every other `secrets --json` envelope is one compact line; the help text now promises one line for all of them. A consumer that parses the whole of stdout as JSON is unaffected; one that read the multi-line layout line by line will see the change.

**Changed -- a `yaw-mcp secrets` refusal in prose names its action once the action is known**

A refusal was labelled `yaw-mcp secrets:` or `yaw-mcp secrets <action>:` depending on which check caught it -- the empty-value refusal read `yaw-mcp secrets set: Secret value cannot be empty.` from the argument parser and `yaw-mcp secrets: Secret value cannot be empty.` from the command. Every prose refusal made once the action is known now reads `yaw-mcp secrets <action>: ...`; one made before it -- an unknown or missing action, or a bad flag ahead of the action, as in `yaw-mcp secrets --bogus set X` -- still reads `yaw-mcp secrets: ...`. The ones that change: the argument errors caught after the action (`unknown flag`, `--value requires a value`, `--secret requires a value` / `--server requires a value` and `unexpected positional argument`, so `secrets set X --bogus` now reads `yaw-mcp secrets set: unknown flag "--bogus"`); `No secret named ...` (`get`, `remove`); `Passphrase required ...` and the unlock errors (wrong passphrase, damaged marker) for `get`, `set` and `remove`; `get`'s decrypt failure; the empty-value refusal (`set`); and `Cancelled.` on every action that prompts. The `--json` envelopes are unchanged.

**Fixed -- `secrets set`'s value prompt asks again on an empty entry, and keeps a pasted Tab**

At a terminal, `set` asks for the value only after it has the passphrase (typed, or from `YAW_MCP_VAULT_PASSPHRASE`) and has derived the key. An empty value -- a bare Enter, or ^D -- refused the whole command at once, and a re-run paid for the passphrase and the derivation again. The value prompt now asks again, as the passphrase prompts do, writing `Secret value cannot be empty.` after each empty entry; after three it refuses as before (`yaw-mcp secrets set: Secret value cannot be empty.`, exit 1, nothing written). ^C still cancels, with exit 130. The prompt also dropped a Tab along with the other control characters, so a pasted token containing one was stored without it, behind a `Stored secret` line and with nothing on the no-echo line to show it. A Tab is now kept at the value prompt, as a piped value always kept it. The passphrase prompts still drop it: a vault created by typing a Tab there is keyed under the string without it, and the same keystrokes have to keep opening it.

`secrets --help` now describes `set` as it behaves: at a terminal it prompts; a piped stdin is read whole, minus one trailing newline, with no flag; and `--stdin` reads stdin even when it is a terminal (raw, multi-line, until EOF, and the terminal echoes it). For scripting it points at piping the value in, not at `--stdin`.

## 1.0.13 -- Codex CLI works again: `install codex-cli` under smol-toml 1.9.0, and Codex can call the tools it loads

Published 2026-09-24, the day after 1.0.12. Six entries below. The first three were reproduced against real Codex 0.144.0 and 0.156.1, and the doubled-BOM refusal was measured against Codex 0.144.0. `mcp_connect_exec` reaches a cached tool from the first call of a session. On a fresh install, `find_tool` and `exec` wait for the startup pre-warm. Codex, which never re-lists tools, is told to call loaded tools through `mcp_connect_exec` (#172). The TOML codec accepts smol-toml 1.9.0's null-prototype tables, which had broken `install codex-cli`, `doctor`, `install --list` and `heal` for every Codex config. It also refuses a doubled BOM the way Codex does, and pins the parser to 1.9 patch releases (#171). The sixth is the entry this release shipped without, written after it was published: a child server's missing credential is typed into a masked field on the loopback page the vault prompt uses, not into a visible form field (#170).

**Fixed -- `mcp_connect_exec` reaches a cached tool from the first call of a session**

`start()` hydrated the persisted tool cache but never built the routing table from it. The later rebuilds all wait on an activation, a config change, auto-load, or a pre-warm that learns something new, so on a machine whose servers were all already cached the table stayed empty. exec looks its steps up there, so the documented "a cached-but-unloaded server is loaded on first use" answered `Unknown tool: <name>. Use mcp_connect_discover ...` instead, until an unrelated activation rebuilt it. The same empty table made a direct `tools/call` on a cached tool that `tools/list` advertises under `YAW_MCP_TOOL_EXPOSURE=full` answer `Unknown tool` as well, and with `YAW_MCP_PREWARM=0` the table stayed empty whatever was cached, since the pre-warm returned before rebuilding anything. The table is now built once `start()` has the config, the cache and the grades. exec is the only way a client that never re-lists tools reaches an upstream tool, and typed's lite `find_tool` -> `exec` flow hit the same wall.
...
smol-toml 1.9.0, published 2026-09-22, returns every table it parses with a null prototype (its release notes: "the objects returned by the library now have a _null prototype_"). The check that decides whether a parsed TOML value is a table accepted `Object.prototype` only, so under 1.9.0 every table read as "a object, not a table" -- the document root included. On a `config.toml` that Codex itself loads, `doctor` reported ``Codex CLI (user): exists but TOML is malformed -- install refuses to overwrite it; fix the TOML by hand, or move the file aside, then run `yaw-mcp install codex-cli` `` and ended on `Warnings above need attention.` (the "Warnings need attention." in Yaw Terminal's MCP panel), `install --list` showed the row as `malformed`, and `heal` listed the file under `could not be checked`, so a dead entry in it was never re-pointed. Moving the file aside, as the warning said, did not help: `install codex-cli` refused to write even a new file -- `failed to splice the "mcp" entry into ...config.toml (...config.toml could not be created (the edit left "" as a object, not a table)). Refusing to overwrite.`, exit 1.

The dependency range was `^1.8.0`, so every install resolved after 2026-09-22 got 1.9.0 -- `npx -y @yawlabs/mcp@latest`, a fresh `npm i -g`, and the copy Yaw Terminal 2.1.7 bundles -- while this repo's lockfile kept the suite on 1.8.0, where it passed. Run against 1.9.0, 170 of the 267 tests in the TOML codec, Codex CLI and heal suites fail. The check now accepts either plain-object prototype (arrays and `TomlDate` still fail it), and a test pins both prototypes directly instead of through whichever version the lockfile resolves. The range moves to `~1.9.0`: the suite runs on the parser users get -- 1.9.0 also carries the fix for GHSA-r4xh-jqrq-34v2 -- and a future minor release cannot reach users untested the way 1.9.0 did. JSON and JSONC clients were never affected: their files go through jsonc-parser. Yaw Terminal users get the fix when the app bundles this release; until then `npx -y @yawlabs/mcp@latest install codex-cli` works once this version is published.

**Fixed -- a `config.toml` opened by two byte-order marks reads as malformed, as Codex reads it**

smol-toml 1.9.0 also strips one leading BOM itself. This codec already strips one before parsing, so a file opening with two BOMs -- which an editor or a PowerShell 5.1 re-encode can leave -- began to parse, while Codex 0.144.0 refuses it (`failed to load configuration`). `doctor` and `install --list` called that file healthy, and `install` wrote into a config Codex will not load. The parse now refuses a second BOM with `a second byte-order mark (U+FEFF) opens the file; Codex refuses it -- delete it`. One BOM, which Notepad writes by default, reads as before.

**Fixed -- a child server's missing credential is typed into a masked field, not a visible form field**

When a local server fails to start and its stderr names a credential it needs (`GITHUB_TOKEN is required` and the like), the broker asks for the value in-session and retries. It asked with a form-mode MCP elicitation carrying one string field per missing key, and a client renders that as an ordinary visible text input: an API token sat on screen, in plain text, while it was typed. The MCP specification (2025-11-25) says servers must not use form mode for passwords or tokens. The 1.0.11 entry for the vault passphrase prompt named moving this prompt onto the same page as a follow-up; this is that follow-up (#170, closing #168).

The value is now typed on the one-shot page the vault prompt uses, served on `127.0.0.1`: one masked field per missing key, labelled with the key's name, with the same hardening and the same 3-minute expiry. The page takes one submission. A POST with a field missing or empty is answered 400 `Every field is required.` and the page stays open, so every value arrives non-empty. A client that declares URL-mode elicitation gets a URL-mode prompt for the page, and is sent `notifications/elicitation/complete` when the page takes a value or expires. A client with form mode only gets a form with no fields, as a yes/no consent, and on yes the broker opens the page in the default browser. A client that declares no elicitation is not asked, as before. The prompt and the page both name the server and the missing keys, and both say to set them in the server's `env` in `~/.yaw-mcp/bundles.json` to skip the prompt in future sessions. While the page waits, a client that asked for progress is told every 5 seconds how long the page has left, as with the vault prompt. Shutdown while the prompt is up refuses the activation with the usual `yaw-mcp is shutting down` message.

What is typed goes into that one server's env, merged over its `env` from bundles.json, for the retry and for every later launch of that server this session. It is held in memory only: never written to disk, never put in the vault passphrase's slot, and never passed to another server. The retry, and the message when the server still names the same keys missing after it, are unchanged.

Each server still gets at most two asks per session, and an ask is counted before anything is shown, however it ends. New in this release is a latch, set by how an ask ended, that stops the asking for that server before the budget runs out:

- A decline (or a cancel) is a decision: no further ask for that server this session. This activate and every later one return the plain `Failed to load "<server>": ...` error.
- A page that cannot start, or a browser that cannot be opened for it on a form-only client, is a wall a second ask would hit identically, and latches the same way. The activation fails with `Could not load "<server>": yaw-mcp could not start the local page that takes the credential in a masked field, so GITHUB_TOKEN is still missing.` -- or `yaw-mcp could not open a browser on this machine for the page that takes the credential in a masked field` -- followed by `Activate "<server>" again for a new page, or set GITHUB_TOKEN in its "env" in ~/.yaw-mcp/bundles.json to skip the prompt in future sessions.` Every later activate returns the same message, and opens no page.
- An expired page does not latch: the user may simply have been away. The activation returns the plain `Failed to load "<server>": ...` error, and the next activate opens a new page while the server has an ask left.
- An elicitation request that errors or times out, or a page whose listener fails after it has started listening, does not latch either: the ask is spent and the activation returns the plain error.

The plain error is booked as a failed activation, which `discover`'s failure warning and `dispatch`'s down-ranking read; the two wall refusals are not.
...
The vault at `~/.yaw-mcp/secrets.json` holds AES-GCM ciphertexts under a key derived from the passphrase alone (scrypt, `src/secrets-crypto.ts`); there is no server, no recovery key and no email, so a forgotten passphrase means the secrets are gone -- by design. What was not by design is that nothing said so and nothing led back to a working vault: every `secrets set` answered `wrong passphrase for this vault (decryption failed)` and exited 1, nothing pointed at the way out, and the only recovery was knowing to delete the file by hand and re-enter every secret from memory, without a list of what was in it (#167).

`yaw-mcp secrets reset` is that way back. It prints the entry names the vault holds (plaintext keys, so no passphrase is needed -- they are the list to re-enter), asks for the word `RESET` on a TTY (any case; a reflexive `y` is a no) or takes `--force` from a script, and without `--force` refuses with exit 2 when stdin or stdout is not a TTY, the way `remove` does. It moves the old file aside as `secrets.json.reset-<timestamp>` next to the vault (the ISO instant with its colons replaced by dashes, since `:` is illegal in a Windows file name; `-2`, `-3` follow if the name is taken) so a passphrase remembered later still opens it, and writes an empty vault under a new passphrase -- confirmed twice on the TTY under rotate's `New vault passphrase` labels, never the bare `Vault passphrase` an unlock prompt shows, or `YAW_MCP_VAULT_PASSPHRASE` when it is set. The new vault carries its verification marker from the start, so the passphrase chosen at reset is the one every later command verifies against; an empty vault without the marker would have accepted anything and let the next `set` establish a different one. It refuses, with the vault untouched, when the passphrase it is given already opens the vault -- whether that came from the env var (refused before RESET is asked for) or was typed from memory at the new-passphrase prompt -- because moving a working vault aside for an empty one under the same passphrase is pure loss; `rotate` and `remove` are what that user wants. When the passphrase is right and only the vault's check marker is damaged -- the state every command refuses with `verification token ("check") is corrupt` -- it refuses the same way and prints the hand-fix (delete the `check` key; the next `set` re-stamps it) rather than pointing at a `rotate` that refuses that vault too. A vault the other commands refuse as corrupt can be reset too: the names are read leniently, and when they cannot be read the output says so instead of listing nothing.

The old vault is copied aside and the new one written over it through the same atomic write every other action uses, so the vault path is never absent, a failed write leaves the old vault exactly as it was (the copy is removed, or named if it cannot be), and a symlinked vault is written through as `set` writes it rather than severed. The changed-on-disk guard the other mutating actions run before their save runs here too, after the prompts. Afterwards a running yaw-mcp server keeps the passphrase it started with and needs a restart (or its in-session prompt answered with the new one), and a `YAW_MCP_VAULT_PASSPHRASE` in a client config's `env` block still holds the old value; the command says both on stderr, as one `{"warning":"vault-reset"}` line under `--json`. `--json` reports `{ ok, path, moved_to, entries, unreadable }`, with `entries` null and `unreadable` the reason when the names could not be read. `secrets --help` and the README gain a "Forgot the passphrase?" paragraph, shell completion offers the action, and the wrong-passphrase error and the corrupt-entry hint now name `secrets reset` as the way to start over.

**Added -- `doctor` says whether the passphrase it can see unlocks the vault**

The SECRET VAULT section reported the passphrase as set or not set, so a stale or mistyped `YAW_MCP_VAULT_PASSPHRASE` -- the state the broker refuses a spawn over with "does not unlock the vault" -- read as healthy there. The line now has four states: set and unlocks the vault; set but does NOT unlock it, with the pointer at the client config's `env` block (doctor sees this shell's, which can differ) and at `secrets reset` if the passphrase is forgotten; set and right, but the vault's check marker is corrupt, so nothing opens until the `check` key is deleted by hand and the next `set` re-stamps it (never a pointer at `reset` for a vault that needs one key removed); and set but unverifiable (no readable vault, or one with no marker and no entries, which accepts any passphrase). It costs one scrypt derivation when a passphrase is set and the derived key is dropped at once; the section stays informational and the exit code does not move. `--json` gains `vault.passphraseUnlocks` (`true`, `false` or `null`) and `vault.checkMarkerCorrupt`. doctor and `reset` share one three-valued check for "is this the vault's passphrase" (`checkVaultPassphrase` in `secrets-vault.ts`: opens, marker-corrupt, wrong) and one wording of the marker hand-fix, rather than each carrying a copy of "unlock, but read the damaged-marker error as a right passphrase".

**Fixed -- under `--json`, the short-passphrase warning and the fresh-vault nudge are JSON lines**

`secrets --help` promised that stderr under `--json` is one JSON object per line, and the schema-behind notice kept it. Two lines did not: the "shorter than 12 characters" warning and the guidance printed when `set` creates the vault were prose, and the first fires at the passphrase step -- ahead of a `{"ok":false}` envelope from the changed-on-disk re-check or a failed write -- so a wrapper parsing stderr broke on a warning rather than on anything the command did. They are `{"warning":"short-passphrase",...}` and `{"warning":"vault-created",...}` lines under `--json` now, prose otherwise, and the help text lists what the warning lines cover.
...
1.0.7 added a fire-and-forget pass on `serve` start that was to run `npx -y <pkg>@latest --version` once per configured npx server, so the first activation of a session did not pay the cold-cache tax, and documented it here and in the README with `YAW_MCP_AUTO_PREWARM`, `YAW_MCP_MAX_AUTO_PREWARM` and `YAW_MCP_AUTO_PREWARM_TIMEOUT`. The call passed no server list, so the pass saw zero npx packages and returned on every start: it has not primed a cache on any machine, and the README's "once-per-startup pre-warm" was an outbound call that did not happen. Wiring it as written would have spawned up to twenty parallel `npx` children -- a registry hit and a package boot each -- on EVERY broker start, and Yaw Terminal starts one broker per pane; and its stated goal was unreachable from that point anyway, because `server.connect` had already completed before the pass fired. The module, its three variables and the claims are gone. Setting any of the three variables now does nothing, which is exactly what it did before. A once-a-day warm, if one is wanted, belongs in an explicit verb the app runs at launch, not on the serve hot path.

**Fixed -- `YAW_MCP_AUTO_HEAL=false` turns the startup heal off, and `=0` does so from `cmd.exe`**

The five `YAW_MCP_*` opt-outs that ship on by default -- `AUTO_UPGRADE`, `SIDECAR_REFRESH`, `AUTO_HEAL`, `PREWARM` and `CONFIG_RELOAD` -- each parsed their variable with their own copy of the check, and each copy carried a comment promising to match the others. They did not. `YAW_MCP_AUTO_HEAL` accepted only a bare `0`, so `=false` left the heal running; and `AUTO_HEAL`, `AUTO_UPGRADE`, `SIDECAR_REFRESH` and `CONFIG_RELOAD` compared the value untrimmed, so the `0 ` (trailing space) that `cmd.exe`'s `set VAR=0 && yaw-mcp serve` delivers was not an opt-out for any of them -- silently, since the variable reads as set everywhere the user can look. All five now go through one parser: `0` or `false`, any case, surrounding whitespace ignored, turns the feature off; anything else -- unset, empty, `1`, `true`, `no`, `off`, `00`, `0abc` -- leaves it on. Nothing that turned a feature off before stops doing so.

**Fixed -- the startup heal inspects the same client configs `doctor` does: env redirects, and every Cline copy**

The stale-entry sweep that runs on every `serve` start (and behind `yaw-mcp heal`) built its list of client config files from its own options alone, and neither caller supplied any -- so under `CLAUDE_CONFIG_DIR` (every Yaw Terminal pane), `CODEX_HOME`, a redirected `%APPDATA%`, `CLINE_MCP_SETTINGS_PATH` / `CLINE_DATA_DIR` / `CLINE_DIR` or `CONTINUE_GLOBAL_DIR`, it inspected the DEFAULT path, printed `No stale yaw-mcp entries found`, and `doctor` went on flagging the dead entry in the redirected file. It now reads every client redirect through the one reader `doctor` and `install` use, and chooses `%APPDATA%` the way they do. It also walks every file a client declares rather than the first: Cline is one (client, scope) fanned out to a shared file plus one copy per editor its extension has run in, and `install` writes all of them, so a dead entry in an editor copy was never repaired. `heal --json` can therefore now list more than one `cline` entry in `healed`, one per file.

**Changed -- the oam floor is the last VERIFIED oam release, and `release.sh` no longer moves it**

`MIN_OAM_VERSION` moved 0.16.1 -> 0.16.2 -> 0.16.3 in five days (1.0.6, 1.0.9, 1.0.10) because `release.sh` read GitHub's latest oam release before every release and moved the floor to it in a commit of its own, running nothing -- each of those blocks says so. Every move cost every machine that had not run `oam self-update`: its node/npx sidecars fell back to node with a warning, and `install` and `heal` wrote npx entries for the broker instead of oam ones -- the slow spawn shape oam hosting exists to avoid -- for oam releases that changed nothing the hosting mechanism depends on. typed users paid the same fallback, since typed's preload launches the same broker binary.

The floor is now the last oam release the hosting mechanism was verified on, and `npm run verify:oam-floor` (`scripts/verify-oam-floor.mjs`) is the only thing that raises it. It hosts a real stdio `@modelcontextprotocol/sdk` server (`scripts/oam-floor-probe-server.mjs`) on this machine's oam through `oam run` and completes initialize + tools/list + tools/call. With no flag it passes when the installed oam is at or above the floor, and fails -- naming the fix -- when oam is absent (the installer; the script installs nothing itself), when `OAM_BIN` names a path that does not exist, when oam cannot host the server, or when it is below the floor (`oam self-update`). With `--raise` it also moves `MIN_OAM_VERSION`, the ratchet literal in `oam-spawn.test.ts` and a CHANGELOG block to the installed version -- all three validated before any is written, a prerelease refused, nothing committed. `release.sh` runs the check (never the raise) in step 1 beside lint, typecheck and tests, reads nothing from GitHub, and commits nothing ahead of the version bump; `SKIP_OAM_FLOOR_VERIFY=1` is its documented last resort for a release machine without a working oam, and `ALLOW_STALE_OAM_FLOOR` is gone with the GitHub read it excused. `OAM_BIN` picks the binary the check runs, as it does for the broker.

The floor itself does not move in this release: 0.16.3 passed the check on 2026-09-21. What changes for a user is that the next oam release no longer moves it on its own.

**Fixed -- this file and the README catch up with 1.0.3 through 1.0.10**

The newest heading here read `## Unreleased` from 1.0.3 through 1.0.10, so eight releases shared one section with no version boundaries, and 1.0.9's `heal` work -- three commits, a new verb and a new environment variable -- was in neither this file nor the README. The sections below are cut at the dates `git tag` records; 1.0.4 and 1.0.9 are written late, and say so. The 1.0.8 entry said the broker never reads `YAW_MCP_READONLY_DIAGNOSTICS`; that stopped being true in 1.0.9, and the entry now says what reads it. The README gains `yaw-mcp heal` in the CLI section and `YAW_MCP_AUTO_HEAL` in the environment table, and its `YAW_MCP_READONLY_DIAGNOSTICS` row names the heal pass among what the variable turns off.

**Fixed -- one slow `oam --version` no longer hosts every sidecar on node until the broker restarts**

The oam probe (`oam --version`, 3 s) runs once per process and its answer is kept for the life of that process. On a saturated machine a probe can lapse while oam sits on disk working -- under one such load `node --version` overran the same 3 s -- and that one lapse hosted every node/npx sidecar on node until the broker restarted, with one warn to say so. A timed-out probe is now retried once before the answer is decided, the way the uv PATH probe already is. Only a timeout is retried: an absent oam (ENOENT), a non-zero exit and a binary that cannot be executed are real answers and are cached on the first attempt, so a machine without oam pays nothing. The retry runs inside the one in-flight probe that concurrent connects share, and before the connect timer is armed, so it is not charged against `MCP_CONNECT_TIMEOUT` or a server's `connectTimeoutMs`.

What changes on a machine whose oam really is wedged: the first probe of a process -- the first connect of a session, or a `yaw-mcp doctor`, `install` or `sidecars install` run -- waits two 3 s windows instead of one before falling back to node, and leaves two detached probe children instead of one. The warn now reads ``oam did not respond to --version twice; falling back to node for this process``; a first timeout that the retry answers logs at debug only, since nothing fell back. There is no switch for the retry on its own: `"runtime": "node"` on a server, or `YAW_MCP_DEFAULT_RUNTIME=node` / `defaultRuntime` in `bundles.json` for every server that has not opted into oam, skips the probe for those servers, and both windows with it, exactly as before.

**Fixed -- the heap-cap hint stops naming a 4 GiB default that a memory-limited container does not have**

When an oam-hosted server dies with `error[OAM-RT-OOM]`, the message named the fix and said `(oam defaults to 4 GiB)`. That is oam's default only outside a Linux cgroup memory limit: inside one, oam caps the heap at 75% of the limit whenever that figure lands between 128 MiB and 4 GiB, so in such a container the number was wrong. The hint now reads ``The oam-hosted child hit its V8 heap cap. Raise it with OAM_MAX_HEAP_MB=<mb> in this server's env, or set "runtime": "node" for this server in bundles.json.`` and names no default at all: oam's own banner is the last thing on the stderr tail the same message carries, and it states the cap that applied and where it came from (the env, the container limit, or the default). The `OAM_MAX_HEAP_MB` row in `yaw-mcp --help` and in the README says `4 GiB by default; can be lower inside a memory-limited Linux container` instead of dating the default to oam 0.9.2, which no oam at or above the floor predates.

**Fixed -- the below-floor advice says a running broker has to be restarted after `oam self-update`**
...
**Changed -- the oam floor moves to 0.16.3**

`MIN_OAM_VERSION` tracks the latest oam release as policy, and v0.16.3 is now current (published 2026-09-20); the floor was 0.16.2. A machine whose oam is older hosts its node/npx sidecars on node instead, and logs a warning naming both versions, `oam self-update` as the fix, and that yaw-mcp needs a restart afterwards. `release.sh` moved the floor and wrote this block; it did not re-run the oam hosting check that `src/oam-spawn.ts` describes.

## 1.0.9 -- stale broker entries heal themselves, `install` writes a path that survives an app upgrade, and doctor sees through fish and PowerShell wrappers

Published 2026-09-20. This section is written late, in 1.0.11: the release shipped with only the floor block below in this file, and nothing in the README, for the three changes above it.

**Fixed -- `install` writes a broker entry path that survives an app upgrade**

When it can host the broker on oam, `install` persists an absolute path to the broker's own entry file. It derived that path from `import.meta.url`, which Node has already realpathed: a broker launched through `...\scoop\apps\yaw\current\...` reported `...\scoop\apps\yaw\2.1.5\...`, and the version directory is exactly what the app's updater deletes as it installs the next release. So every entry written from inside Yaw Terminal named a file that stopped existing on the next upgrade -- and `oam run` cannot refetch a missing entry file the way `npx` can, so the client could not start the broker at all. It is also why `~/.claude.json` kept working while `~/.codex/config.toml` did not: the app spawns the sidecar by the junction path, so `process.argv[1]` carries it, and the app writes the Claude Code entry from that.

The path `install` persists now goes through `stable-entry.ts`, which recovers a spelling of the same file that the updater maintains rather than deletes. Two strategies, cheapest first, both gated on realpath identity so a wrong guess can only ever be a no-op: prefer the `process.argv[1]` spelling when it names the same file (exact, no directory scanning, and it covers the bundled-app case outright); otherwise substitute a sibling symlink for a version-shaped ancestor -- scoop's `current`, an fnm or volta `default`, any `/opt/<app>/current` layout -- with no per-installer table. The walk is deepest-first and substitutes only for a version-shaped segment, because a shallowest-first walk that accepted identity alone rewrote `C:/Users/...` to `C:\Documents and Settings/...`, a hidden compatibility junction whose ACL denies traversal to most processes. A per-launch mount -- an AppImage under `.mount_*` or, without FUSE, `appimage_extracted_*`; a translocated macOS app -- is refused outright, since no spelling of it is durable and no later repair could converge; those installs stay on the npx entry, which re-resolves per spawn.

**Added -- `yaw-mcp heal`, and a startup pass that re-points stale broker entries (`YAW_MCP_AUTO_HEAL=0` turns the pass off)**

The fix above stops new entries rotting. It does nothing for the ones already on disk, and nothing in the product rewrote a client config on upgrade or at startup -- `doctor` said so in its own words. This is that missing rewrite, on the discipline Yaw Terminal already proved for Claude Code: an absolute path is safe to persist only because the writer can recognise its own previous output and refresh it. Four gates must all pass before a byte is written: the entry is an `oam run` launch whose entry file sits inside a `node_modules/@yawlabs/mcp/` tree (a hand-written entry, a pinned `--package` form, a bare `oam`, or an oam entry for somebody else's server is invisible to the sweep); the entry path is not a regular file (a working entry is never touched, however unusual it looks); `YAW_MCP_READONLY_DIAGNOSTICS` is not set (see below); and the entry was not written for another OS (a WSL session reading a Windows profile passes the shape check and then fails the file check for the wrong reason, because `statSync("C:\...")` from Linux throws -- so `doctor`'s foreign-launch rule is a gate too, checked before the file check, in both directions). The second gate is what makes the pass converge: it is a no-op the instant the entry resolves, so it cannot fight another writer or rewrite in a loop.

Two triggers, because neither reaches everyone alone. `serve` runs the sweep fire-and-forget at startup, cross-client by design: the client with the dead entry cannot start the broker, so the repair rides in on a client whose entry still works -- on a Yaw box the app keeps the Claude Code entry live, so a pane spawn is what heals Codex. `yaw-mcp heal [--json] [--dry-run] [--quiet]` is the explicit verb for the case that cannot reach: a user whose only configured client is the broken one, and the command Yaw Terminal runs at app startup for exactly that reason. `YAW_MCP_AUTO_HEAL=0` turns the startup pass off (the same shape as `YAW_MCP_AUTO_PREWARM`), and does not apply to the verb, which the user asked for by name. `YAW_MCP_READONLY_DIAGNOSTICS` applies to both: the flag exists so a diagnostics poll nobody asked for writes nothing, and the heal pass reads it on every `serve` start and writes nothing while it is set -- which is what makes the 1.0.8 sentence "the broker never reads the variable" false from this release on; that entry is corrected below.

The rebuilt entry goes through the same `buildLaunchEntry` `install` uses, so it is byte-identical to what `install` would write today, and carries the stored entry's `env` and client-owned fields forward -- Codex's `startup_timeout_sec` survives the repair. When no durable path resolves it falls back to the npx entry, which never dangles; on linux-arm64, where oam publishes no binary, that is always the answer. oam is probed lazily, only when a rewrite is about to happen, so the steady state of "nothing is stale" costs no process spawn on startup. `heal` prints `No stale yaw-mcp entries found.`, or each re-pointed entry with its old and new path and `Restart the affected client(s) to pick this up.`; a config the sweep could not read into -- an unspliceable TOML inline table, a blocked container, an unreadable file -- is listed separately as `could not be checked`, because "nothing to repair" and "I could not check" are different answers and only one of them means the user is fine. Exit 0 for both a clean sweep and a repair; only an unexpected failure exits 1, reported on stderr, so the app can tell a real failure from an older broker that does not know the verb. `--json` returns `{ healed, unhealable, count, dryRun }`. On the `serve` path stdout is untouched: there it is the JSON-RPC transport, and everything reports through the stderr log. `YAW_MCP_AUTO_HEAL` joins `--help`'s Environment block and `doctor`'s ENVIRONMENT section. A cross-platform pass pinned one layout per shape we ship or a supported host produces -- AppImage (FUSE, relocated `TMPDIR`, extract-and-run), macOS (`/Applications`, Homebrew cask, translocated), Linux (deb/rpm, `/usr/lib`), Windows (scoop, Program Files) -- plus decoys that must not match, and stopped `norm()` case-folding paths anywhere but Windows, where folding `/opt/Yaw` into `/opt/yaw` had let the dedupe key collapse two real entries into one and the convergence guard mistake a different path for the old one.

**Added -- `doctor` and the heal pass recognise fish and PowerShell wrappers around an oam launch**

`install` never writes a wrapped entry -- the `cmd /c` wrap exists for npx's `.cmd` shim, and oam is a real executable -- but a hand-edited config reasonably might, and a broken entry wrapped in `fish -c` or `pwsh -Command` was invisible to both `doctor` and the sweep: it stayed broken and nothing said why. Both recognisers now see through them. fish joins the POSIX branch, since its `-c` takes a single command string exactly like the Bourne family and neither recogniser parses the payload beyond its first whitespace token. PowerShell needed more than a name in a regex: `-Command` takes the rest of the line, not one argument, so the tail is rejoined before tokenising (`pwsh -Command oam run --no-check <path>` works, not only the single-quoted form); any unambiguous prefix names it, so `-c`, `-co` and `-comm` are accepted and `-Confirm` and `-ComputerName` are pinned as not; `-File`, `-EncodedCommand` and a bare script return nothing rather than a guess. The two recognisers still differ on purpose: `isOamLaunch` (the runtime label) strips surrounding quotes and accepts, while `oamArgvTokens` (the entry path `doctor` and the heal pass check) bails on any quote, because half a path fails the exists check and reads as a broken entry, and broken entries are what the heal pass rewrites. The shared predicates now live in one place, so the next shell is a one-place change.

**A behaviour change worth naming**: a broken entry wrapped in fish or pwsh is now repaired, and the repair writes the shape `install` writes, so the hand-rolled wrapper does not survive it. Before, such an entry was left alone and stayed broken. Only a non-starting entry is ever touched; a working pwsh-wrapped entry is pinned as untouched. `YAW_MCP_AUTO_HEAL=0` is the way to keep the startup pass off such an entry entirely.
...
## 1.0.8 -- `doctor` and `bundles` can run without writing anything

Published 2026-09-17.

**Changed -- the routing advice moves out of the meta-tool descriptions and into the server's MCP `instructions`**

The eleven `mcp_connect_*` meta-tools were 17,929 chars of `tools/list` wire JSON (about 5.1k tokens), 10,849 of them description text, and three of them -- discover, dispatch and exec -- each repeated the same routing advice: prefer dispatch for a concrete task, discover only to browse, read the guide first, load only what the task needs. That text was paid on every `tools/list`, and on a client with no deferred tool loading it was inlined into every request. The broker now sends it once, as the `instructions` string in the MCP `initialize` result (Claude Code puts it in the system prompt as a "MCP Server Instructions" block), and each description says only what its tool does. Measured after: 11,590 wire chars, 5,456 of description; `mcp_connect_exec` went from 3,897 to 2,806 (its description from 2,162 to 1,493). Both totals are pinned in `meta-tools.test.ts` with a ceiling so a description cannot silently regrow. No tool name, schema property, annotation or refusal shape changed; the constants the descriptions quote (`MAX_EXEC_STEPS`, the reliability floor, `YAW_MCP_IDLE_THRESHOLD`) are still interpolated, not retyped.

**Added -- a `lite` tool exposure, picked by default for typed-cli**

`YAW_MCP_TOOL_EXPOSURE=lite` advertises only `mcp_connect_exec`, `mcp_connect_find_tool` and `mcp_connect_read_tool` (plus the servers loaded this session, gated exactly as under `gateway`). Between them those three reach any installed tool without the client's tool list changing -- find_tool names it, read_tool describes it, exec calls it and loads its server on first use -- which is the set typed-cli's own gateway logic already keeps when it has to cap. The other eight meta-tools are not listed but stay callable by name: lite narrows what `tools/list` advertises, not what `tools/call` accepts.

With `YAW_MCP_TOOL_EXPOSURE` unset the broker now reads the client's `clientInfo.name` from the `initialize` request and picks `lite` when it is `typed-cli`, `gateway` for everything else. typed-cli inlines every advertised tool into every request and has no deferred loading, so all eleven meta-tools rode every turn on its hosted tiers; and in the usual Yaw Terminal setup its yaw-mcp entry comes from `~/.claude.json`, which typed cannot add an env var to, so the broker is the only place that could decide this. A set `YAW_MCP_TOOL_EXPOSURE` always wins over the client default -- `gateway` pins a typed-cli session to the full meta-tool set, `lite` opts any other client in -- and an unrecognised value now falls back to that client's default (the warning names which) rather than always to `gateway`. `yaw-mcp --help`, `doctor`'s ENVIRONMENT hint and the README env table describe all three values.
...
The broker's startup config load does not read the variable: it still migrates legacy files even when the variable has leaked into a client's environment, because it is the process that folds them in. (This paragraph used to open with "The broker never reads the variable", which was true of 1.0.8 and stopped being true one release later: since 1.0.9 the broker's startup heal pass reads it on every `serve` start -- gate 3 of that pass, see 1.0.9 -- and writes no client config while it is set. `yaw-mcp heal` honours it the same way. Corrected in 1.0.11.)

## 1.0.7 -- a startup pre-warm of `npx -y <pkg>@latest` servers

Published 2026-09-16. Besides the block below, the README's intro was edited (1af6a19, a one-line change with no entry of its own).
...
`prewarmDormantServers` (the existing startup pre-warm) does spawn each server at startup, but (a) it fires from `oninitialized`, so the FIRST activation of the session still pays the cold-cache cost -- the user only sees the win on the second server and onwards, and on the second session onwards; (b) it is gated on `YAW_MCP_PREWARM`, which a user with no pre-warm tolerance can turn off and would not want re-lit by a different knob; and (c) it spawns the full upstream, not just the npx cache prime.

A new fire-and-forget pass on `start()` runs `npx -y <pkg>@latest --version` once per unique configured npx package, in parallel, with a 30s per-package timeout (the same 30s as the client deadline -- a prime that takes longer than the budget it is trying to buy back is not earning its keep). The next real `StdioClientTransport` spawn finds a warm cache and connects in well under a second. The pass is no-op when the user has no npx servers or opts out (`YAW_MCP_AUTO_PREWARM=0`), is capped at `YAW_MCP_MAX_AUTO_PREWARM` (default 20, so a metered connection's worst case is bounded), and shares `acquireSidecarsLock` with `maybeRefreshSidecars` so a manual `sidecars install` running at startup defers this pass and vice versa.

The spawn mirrors `maybeAutoUpgrade`'s env hygiene (`stripInternalSecretsFromEnv(process.env)` so the vault passphrase never reaches an npx child's transitive pre/postinstall), `stdio: "ignore"` so a slow child cannot wedge the parent, and `shell: process.platform === "win32"` so the npx `.cmd` shim resolves. The new `auto-prewarm.ts` module is a sibling of `auto-upgrade.ts` and `sidecar-refresh.ts`; the public surface is `maybeAutoPrewarmNpxCache(deps)`, plus the pure parsers `isAutoPrewarmDisabled`, `parseAutoPrewarmMax`, `parseAutoPrewarmTimeoutMs` and the pure extractor `extractNpxPackages`, all exported for tests and `doctor`.

**The "first activation" caveat remains for the rare user who skips a pre-warm**: a user on a metered link who sets `YAW_MCP_AUTO_PREWARM=0` and then activates a never-touched-before npx server on a cold `_npx` cache still gets the >30s first-spawn tax. There is no on-demand warm without paying the same tax once, and the only way to remove that case would be a one-time payment on the user's first `activate` of the session -- which IS the cold-cache race, just hidden behind a different layer.

## 1.0.6 -- the oam floor moves to 0.16.1

Published 2026-09-15, carrying the floor move below and the version bump: no other change.
...
## 1.0.5 -- `doctor` and `install --list` read a Codex CLI config.toml as TOML

Published 2026-09-14.

**Fixed -- doctor and `install --list` read a Codex CLI config.toml as TOML, not as broken JSON**

`doctor` and `install --list` parsed every client file as JSON, so a valid `~/.codex/config.toml` -- the one `install codex-cli` writes included -- read as `exists but JSON is malformed` in `doctor` and `malformed` in `install --list`. Both now classify a non-JSON file through the same adapter `install` reads it with. A valid `config.toml` with a `[mcp_servers.mcp]` table reads `OK -- has "mcp" entry` in `doctor` and `installed` in `install --list`. JSON and JSONC clients are read exactly as before.

**`doctor`'s exit code changes**: a machine whose only warning was that Codex CLI line now exits 0. Once Yaw Terminal bundles this release, that clears its MCP panel's "Warnings need attention." **`install --list`'s STATUS for the Codex CLI rows moves off `malformed`** to `installed`, `no-entries` or `other-entries`, as the file says.

A `config.toml` that really does not parse is reported in TOML words by `doctor`, `install` and `import`: ``exists but TOML is malformed -- install refuses to overwrite it; fix the TOML by hand, or move the file aside, then run `yaw-mcp install codex-cli` ``. A `mcp_servers` key that is not a table reads `not a TOML table` and `make it a table (or remove the key)`. An `mcp` entry written as an inline table, as dotted keys, as an array of tables, or inside an inline `mcp_servers = { ... }` is reported as present but not editable -- install will not rewrite it, and doctor cannot check its launch command -- with the by-hand step for that spelling, and is not a warning. The step is per spelling because no one step fits all four: an entry inside an inline `mcp_servers` can neither be rewritten as its own `[mcp_servers.mcp]` table (a redefinition, which stops the file loading) nor deleted and re-installed (the header install writes is refused by the still-inline container); only converting the container works, and the line says that. Where the step is a new `[mcp_servers.mcp]` table, it says to put the table at the end of the file: written where the old line was, it would take in the keys below it, and the next install would delete them. An inline `mcp_servers = { ... }` holding other servers and no `mcp` entry reads as present with no `mcp` entry and names converting the container as the step, rather than `run install`, because an install run exits 1 on that file.
...
## 1.0.4 -- review follow-ups to the Windows lock retry

Published 2026-09-13, three hours after 1.0.3, and written late, in 1.0.11: the release corrected two figures in the 1.0.3 entry below and added nothing of its own here.

**Fixed -- the Windows lock retry covers the self-upgrade lock's restore, and its figures are measured rather than quoted**

An adversarial review of the 1.0.3 lock change found no bug in the retry itself, and six things that were false or weaker than claimed. The retry on the self-upgrade lock's steal path -- the create that puts a live holder's lock back -- had no test at all: reverting it to a bare `openSync` left all 99 auto-upgrade tests green, while the 1.0.3 commit said every part of the fix had been mutated. A test now injects `EPERM` on that restore and asserts the holder's lock comes back. The grade cache's budget-restart test removed the lock with a real `rmSync` from the test body, so under load an open already in flight could re-create it after the unlink and the second `EPERM` was never thrown -- the mutant that drops the budget reset survived 4 of 80 loaded runs -- and it hard-coded a 1300 ms wait against a private 1 s budget, so raising the constant would have switched the test off. The budget is now a test hook the test passes in, the wait is a `setTimeout`, and the release and the `EPERM` are one scripted step inside the mocked open; the mutant is killed in 15 of 15 runs under load.

Three comments and the 1.0.3 entry carried numbers that did not survive measurement. "225 times in 2000 races" came from one high run and counted failed opens, not races: over 20 runs the count ranged from 20 to 225, most between 40 and 110, and the failure also reproduces across processes. "26 ms in all, paid only when every attempt fails" was wrong on both counts: the three waits measured 31-74 ms idle on Windows and up to about 300 ms under load, they are paid whenever the first three attempts fail, and one acquire can pay them twice. The grade cache's retry-branch comment said the create had failed; it also runs when the create succeeded and the token write or close after it failed. The 1.0.3 entry below carries the corrected figures.

## 1.0.3 -- a fresh Yaw Mode pane names the local-scope entry it will lose, and an audit stops failing on the lock the one before it was releasing

Published 2026-09-13, two hours after 1.0.2.

**Fixed -- a fresh Yaw Mode pane warns about a local-scope Claude Code entry, and the augment Keeping line stops promising the entry goes home**

In a fresh Yaw Mode pane, `install claude-code --scope local` wrote the entry into the overlay's `.claude.json` -- `projects[<dir>]` lives in `<CLAUDE_CONFIG_DIR>/.claude.json`, and a fresh pane keeps nothing of its overlay -- and said nothing about it. It now prints: ``Note: a fresh Yaw Mode pane does not keep its config dir, so the Claude Code entry in <overlay>/.claude.json goes when this pane closes. Run `yaw-mcp install claude-code --scope local --project-dir <dir>` from a normal shell to keep it.``

In an augment pane, the line `uninstall` prints when it keeps the `~/.claude/settings.json` grant for a Claude Code entry in the pane's own `.claude.json` said Yaw carries that entry home when the pane closes. It now says Yaw may: ``Keeping mcp__mcp__* in ~/.claude/settings.json: Claude Code (user) still launches yaw-mcp from <overlay>/.claude.json, which Yaw may carry home when the pane closes.`` Yaw skips that copy when its `yawModeSyncClaudeJson` setting is off, and when `~/.claude.json` changed while the pane was open it merges instead, which brings no overlay `mcpServers` entry into a home file that already has `mcpServers`. The grant is still kept, for the case where the entry does arrive.

**Fixed -- on Windows, an audit stops failing on the lock the audit before it was releasing**

A release run died in its test gate: `grades-cache.test.ts`'s concurrent-writes case threw `EPERM: operation not permitted, open '...\.yaw-mcp\grades.json.lock'`. It was not the test. On Windows, creating a file with `O_EXCL` while another thread or process is deleting the same path fails `EPERM` instead of succeeding or answering "already exists". On the machine that failed (Windows 11, Node 22.22.2), a probe racing three creates against one delete 2000 times hit it in every one of 20 runs. The grade cache's lock take treated every errno other than "already exists" as fatal, so a `yaw-mcp audit` whose wait happened to land inside the previous holder's release failed with exit 3, grade computed but not cached. The Yaw Terminal MCP panel grading two servers at once is exactly that shape. The take now retries `EPERM`, `EBUSY` and `EACCES` on Windows -- the codes `atomicWriteFile` already retries its rename on -- for up to a second, then reports the real errno rather than a lock-timeout message. POSIX is unchanged.

The same take backs the `bundles.json` write lock, the sidecars install lock and the self-upgrade lock, and there the misread was quieter: a lock it could not create was treated as "no lock possible here, proceed", so the same `EPERM` ran the write or the install unlocked beside whichever process took the lock next. It now retries those codes three times before falling back to that best-effort answer. The backoffs add up to 26 ms on paper and measured 31-74 ms on an idle Windows machine, since short waits there overshoot.
...
`--repair` keeps the string values exactly as before, and so does a TTY prompt answered `[o]verwrite`, which now names the keys it keeps: `[o]verwrite (keeping env: OAM_BIN, YAW_MCP_VAULT_PASSPHRASE), [s]kip, or [a]bort? (default: skip)`. The off-TTY refusal names the same keys next to `--repair` and says `--force` drops the env.
...
`install --list` printed `other-entries` for every client config that existed, including the empty `{"mcpServers": {}}` that `uninstall` leaves behind, while its help defined the status as "the file exists with other servers in it". The status now comes from the one server list the row reads -- `mcpServers`, VS Code's `servers`, or Claude Code local scope's `projects[<dir>].mcpServers` -- and a list that is absent or empty prints the new `no-entries`. The scoping matters for Claude Code, whose user and local rows read the same `.claude.json`: a user-level server no longer makes the local row claim it, and the help legend now says the status is about the row's list, not about the file. That legend reads as a complete enumeration of what the column can say and was not one: `statusFor` returned seven shapes at 1.0.0 and the legend named three, so `legacy: <key>`, `malformed`, `unreadable: <reason>` and `unavailable` all shipped unnamed, and the `(other drive case)` suffix the drive-letter block below adds would have been a fifth. It now names every one of them but `unavailable`, and that omission is deliberate: `unavailable` is the fallback for a client whose `availableOn` omits an OS while its `notConfigurableOn` carries no reason to print instead, a state no shipped target is in, so naming it would document a status the column cannot print. A test derives the set from `statusFor` and fails when the legend stops covering it, that exclusion included. `doctor --json` gains the count behind it, as an additive `containerEntries` field on each `clients[]` entry.
...
**Fixed -- Claude Code local scope: the `projects[]` key on Windows, and the entries older versions left under the other drive-letter case**

`install claude-code --scope local --project-dir c:/repo` (or a drive-relative `c:repo`) wrote the entry under `projects["c:/repo"]` in `~/.claude.json`. Claude Code looks that up byte-exactly under its working directory as the shell reported it, and PowerShell -- and Git Bash once it has run a `cd` of its own -- report `C:` even after `cd c:/repo`. So install printed Done, `install --list` handed the same directory agreed it was installed, and `/mcp` showed nothing. The drive letter in the key is now upper-cased; the rest of the path, UNC paths and POSIX paths are unchanged.

`uninstall`, `doctor`, `install --list` and `import` now also SEE an entry under the other drive-letter spelling of the same directory, which is what an older version wrote. Each of them used to look under the one key its own arguments spelled, so a spelling that did not match the one on disk read as an absence. On 1.0.0 the spelling a run computed came from `--project-dir` when that was passed, and otherwise -- which is how the command is ordinarily run -- from whatever the shell reported as the working directory. So nothing in either command's own text had to differ for two runs to land on different keys. Measured on a 1.0.0 build: a bare `install claude-code --scope local`, run from a cmd.exe sitting in the project after `cd /d c:\repo`, wrote `projects["c:/repo"]`, because cmd.exe keeps the drive letter as typed, with or without `/d`; the matching `uninstall claude-code --scope local --force` -- same client, same scope, no `--project-dir` on either side -- run from PowerShell in that same directory, looked under `projects["C:/repo"]`, because PowerShell reports an upper-case drive whichever case was typed. The two commands are not the same text, and they do not need to be: neither one names a directory, so each takes the drive case from its own shell. That run stripped the `permissions.allow` grant, printed "Claude Code no longer launches yaw-mcp", and left `projects["c:/repo"].mcpServers.mcp` in place -- still launching the broker for any session whose cwd carried the other spelling -- and a second run then reported "Nothing to do". (Both shells report the directory with backslashes, `c:\repo` and `C:\repo`; the forward slashes are the key's, which is that path with every `\` rewritten as `/`.) Naming the directory did not settle it on 1.0.0 either: `--project-dir c:/repo` and `--project-dir C:/repo` each pinned the key that one run computed, because 1.0.0's key builder rewrote the separators and left the drive letter as it arrived, so spelling the directory out helped only when the second run repeated the first one's spelling. This version upper-cases the drive letter in the key it computes and folds the other spelling on read, so the drive case a shell reports or a `--project-dir` carries no longer decides which entry a run finds. `doctor` and `install --list` reported the project as having no yaw-mcp entry at all. `uninstall` now removes the entry from every drive-letter spelling of the same key in one write, and names the key on the lines for the spellings that are not the canonical one -- the canonical removal still reads `Removed the "mcp" entry.` and a non-canonical one reads `Removed the "mcp" entry under projects["c:/repo"].` -- so its closing all-clear is no longer printed over an entry the same run left behind under the other spelling of the drive letter. `doctor` and `install --list` report the entry and name the key it is under: the STATUS column can now read `installed (other drive case)`, with the key itself printed in a note under the table, since that is the only place a full path fits. A script matching that column exactly will see the new text for a project whose entry is under the other spelling. Only the drive letter is folded: two keys differing in any other character, and POSIX and UNC keys, are still separate projects. `doctor --json` carries the key in a new additive field, `clients[].entryProjectKey`.

`install` is the exception, deliberately. It writes the canonical upper-case key and LEAVES an entry under the other spelling, printing a note that names it -- deleting it would unwire the one kind of session that can read it (cmd.exe, and a Git Bash started from cmd before its first `cd`) and give that session nothing back, since it cannot read the canonical key. Run `yaw-mcp uninstall claude-code --scope local` to clear both spellings, then install again.

Upgrading from 1.0.0, concretely. A local-scope install made with `--project-dir c:/repo` left two things behind: `projects["c:/repo"].mcpServers.mcp` in `~/.claude.json`, and `mcp__mcp__*` in `<repo>/.claude/settings.local.json`. On 1.0.0 which of the two the first `uninstall` cleared came down to how the drive letter was spelled the second time -- by `--project-dir`, or, with that absent, by the shell that reported the cwd -- and the trap is a mismatch rather than any one command. Repeat the install's own spelling -- `uninstall claude-code --scope local --project-dir c:/repo --force` -- and 1.0.0 did the whole job: `Removed the "mcp" entry.`, the settings patch, Done, and an empty container behind it. Uninstall the way the command is ordinarily run, though, with no `--project-dir` and the project as the working directory, and PowerShell -- and Git Bash once it has run a `cd` of its own -- hand it `C:\repo`, which is the key `C:/repo` once the separators are rewritten: 1.0.0 looked under `projects["C:/repo"]`, found nothing there, removed only the permission grant, printed "Claude Code no longer launches yaw-mcp" over an entry that still launched it for any session whose cwd carried the other spelling, and said "Nothing to do" on the second run. `--project-dir C:/repo` reached the same dead end. One run of this version clears both, whichever spelling reaches it -- typed into `--project-dir` or reported by the shell: the entry under whichever key holds it, and the grant.

Two limits worth knowing. The closing all-clear covers the containers the run's own scope reads; a `--scope user` entry at the root of the same `~/.claude.json` is not part of it and needs its own `uninstall --scope user`. And cmd.exe keeps a typed lower-case drive (`cd c:\repo`, with or without `/d`), with a Git Bash started from that prompt inheriting it until its own first `cd` -- a Claude Code started from either looks under `c:/...` and does not read the `C:/...` entry install writes, while `doctor` run from that shell still reports it OK. Start Claude Code from PowerShell, run `cd .` in that Git Bash, or run `cd /d C:\repo` in cmd first.

**Added -- `install typed`, and one permissions grant that two clients share**

`yaw-mcp install typed` wires yaw-mcp into the [typed](https://typed.cloud) CLI, and `uninstall`, `install --list`, `--all`, `doctor`, `try`, `import` and shell completion all take the new id. It writes typed's own `~/.config/typed/mcp.json`, not one of Claude Code's files: the same path on every OS, never moved by `CLAUDE_CONFIG_DIR`, and strict JSON, so install refuses to splice into a copy carrying a comment, as it does for Cline. typed ranks that file above the user-scope files it shares with Claude Code, so its `mcp` entry wins over one Yaw Terminal manages in `~/.claude.json`, while a project's `.mcp.json` still wins over it. When the entry install writes is `npx` (no oam, or no global install for it to point at) and one of those lower-ranked files already launches yaw-mcp locally -- Yaw Terminal's bundled copy is the usual one -- install prints a note: the npx entry replaces that local launch for typed and resolves `@latest` on every typed start, which is slow and can exceed typed's MCP connect timeout, and `npm i -g @yawlabs/mcp` with oam installed plus a re-run, or a larger `MCP_TIMEOUT`, is the way out. On Windows the entry is a bare `npx`, because typed resolves the `.cmd` shim itself. There is no project scope -- `<project>/.mcp.json` is already Claude Code's, and `install mcp` writes it. **It needs a typed CLI newer than 1.5.0**: typed 1.5.0 and older do not read `~/.config/typed/mcp.json` and load only Claude Code's files, and `install claude-code` is the command to run there.

typed reads `permissions.allow` from Claude Code's user `settings.json` (`$CLAUDE_CONFIG_DIR/settings.json` when that is set), so `install typed` adds the same `mcp__mcp__*` grant `install claude-code` adds, to the same file, and typed does not prompt on each yaw-mcp tool call. That grant follows `CLAUDE_CONFIG_DIR` where typed's `mcp.json` does not, so under a set one install prints a note naming the `settings.json` that got the grant and saying it is scoped to that config dir, and `uninstall` prints the matching note about which `settings.json` it looked in. A Yaw Mode pane does not get that note; the Yaw Mode block below describes what it gets instead. Install and uninstall now decide whether to touch that grant from the install target, which declares that it patches Claude Code's permissions, rather than from the client being Claude Code. Because the two clients share one grant, `uninstall` stops removing it while the other still has its entry: `uninstall typed` with Claude Code still wired, or `uninstall claude-code --scope user` with typed still wired, leaves the grant and prints one line naming the client that keeps it, `Keeping mcp__mcp__* in <settings.json>: Claude Code (user) still launches yaw-mcp from <.claude.json> and reads that grant.` It also leaves a grant when the other client's config exists but cannot be read, and when typed still loads an `mcp` entry from one of the other files it reads -- `~/.mcp.json`, `<configDir>/.mcp.json` (where older `typed mcp add` releases wrote) or the top-level `mcpServers` of `~/.claude.json` and `<configDir>/.claude.json` -- other than the entry the uninstall itself is removing. A typed file typed cannot load (a comment, a trailing comma) does not keep it. On Windows the file comparison ignores letter case, so a project folder spelled `c:\Users\...` still matches the home. One case changes for a Claude-Code-only setup: when the project folder's `.claude/settings.json` IS the user-scope `settings.json` -- the project folder is your home directory, with `CLAUDE_CONFIG_DIR` unset or pointing at `<home>/.claude` -- uninstalling either of Claude Code's user or project entries now keeps the grant while the other remains. `install --all` names that shared `settings.json` once in its written and would-write lists, though both clients plan the grant.

The closing lines speak to typed. `install typed` ends `Done: typed is configured. typed picks the entry up in its next session.` -- typed reads its MCP config once, when a session starts, so there is no running app to restart. `uninstall typed` says typed drops the server in its next session, and says that typed no longer launches yaw-mcp *from `~/.config/typed/mcp.json`* rather than at all: typed's CLI preloads Yaw MCP on its own when no config it reads references it and it finds a `yaw-mcp` on PATH or a `~/.yaw-mcp` directory, so the line adds that `TYPED_CLI_NO_YAW_MCP=1` keeps it out. Every other client's install and uninstall Done lines are unchanged, and a test pins them whole for every client yaw-mcp configures on Linux -- all but Claude Desktop -- at every scope each one has.

An older typed CLI ignores `~/.config/typed/mcp.json` without a word, so `install typed` now looks for itself. It reads the bundle typed's launcher runs -- `$TYPED_CLI_BUNDLE`, else `~/.config/typed/typed-cli/cli.mjs` -- and when that file exists and does not contain the quoted `"mcp.json"` path segment typed's loader for the file compiles to, prints one warning to stderr: ``yaw-mcp install: warning -- the typed CLI at <bundle> predates ~/.config/typed/mcp.json, so it will not load this entry until it updates -- run `typed update`.`` `typed update` installs the newest typed release, and the row needs one newer than 1.5.0. With `TYPED_CLI_BUNDLE` set the warning says the bundle has to be rebuilt or replaced instead, since `typed update` refreshes only the default one. It probes the bundle's bytes, never a version number, prints nothing when there is no bundle to read, and changes neither what install writes nor its exit code; it fires on `--dry-run` and on an already-configured re-run as well. `TYPED_CLI_BUNDLE` joins the `--help` Environment block.

**Fixed -- in a Yaw Mode pane, the `mcp__mcp__*` grant outlives the pane**

A Yaw Mode pane sets `YAW_MODE` to `augment` or `fresh` and points `CLAUDE_CONFIG_DIR` at a per-pane overlay, and `install claude-code` and `install typed` at user scope wrote the grant only to that overlay's `settings.json`. Yaw discards that copy with the pane: an augment pane starts it as a hardlink to `~/.claude/settings.json`, but Yaw unlinks it to write its own per-pane allow-list, install's atomic write replaces it either way, and `settings.json` is not among the files Yaw carries home when the pane closes. So every session after the pane went back to asking about yaw-mcp's tools.

With `YAW_MODE` set to one of those two values and `CLAUDE_CONFIG_DIR` naming a Yaw overlay -- a directory whose name starts with `yaw-mode-`, the name every overlay Yaw builds carries and the test Yaw itself applies -- a user-scope install or uninstall of either client now:

- **augment** -- writes the grant to `~/.claude/settings.json` as well as the overlay's, and the `Wrote` line for `~/.claude/settings.json` ends ` -- a Yaw Mode pane's own settings.json does not outlive the pane`. `uninstall` removes it from both, and keeps each file's grant while another client that reads that file still has its entry. For `~/.claude/settings.json` that includes a Claude Code entry in the pane's own `.claude.json`, which Yaw normally carries home, and the line kept for it says so: ``Keeping mcp__mcp__* in ~/.claude/settings.json: Claude Code (user) still launches yaw-mcp from <overlay>/.claude.json, which Yaw carries home when the pane closes.`` That line overstates it -- Yaw does not always copy the entry home; 1.0.3 above corrects it. An entry in any other file under the overlay -- typed's `<overlay>/.mcp.json`, say -- keeps only the overlay's grant, since that file goes with the pane.
- **fresh** -- writes the overlay only, since a fresh pane reads nothing from home, and prints one note naming what goes when the pane closes, because a fresh pane keeps nothing of its overlay. For `install typed`, whose entry is outside the overlay, that is the grant: ``Note: a fresh Yaw Mode pane does not keep its settings.json, so the mcp__mcp__* grant in <overlay>/settings.json goes when this pane closes. Run `yaw-mcp install typed` from a normal shell to keep it.`` For `install claude-code`, whose entry lands in the overlay's `.claude.json`, it is both: ``Note: a fresh Yaw Mode pane does not keep its config dir, so the Claude Code entry in <overlay>/.claude.json and the mcp__mcp__* grant in <overlay>/settings.json go when this pane closes. Run `yaw-mcp install claude-code` from a normal shell to keep them.`` -- printed whenever install writes either file there. `uninstall` removes from the overlay only, without a note.

Project scope is unchanged; the grant files of project and local scope are project-relative, so neither scope gets a second patch or a grant in the overlay. Claude Code's local-scope entry is the exception: it lives in `<CLAUDE_CONFIG_DIR>/.claude.json` under `projects[<dir>]`, so it is in the overlay too, and in a fresh pane it goes when the pane closes -- in this release without a note; 1.0.3 above adds one. Every run outside a Yaw Mode pane is unchanged as well. That includes a `CLAUDE_CONFIG_DIR` of your own set at a pane's prompt: Yaw exports `YAW_MODE` to the pane's shell too, but a directory not named `yaw-mode-*` is not an overlay, and install treats it exactly as it would outside Yaw. In a pane, the lines above replace the `install typed` note about a grant scoped to `CLAUDE_CONFIG_DIR`. `YAW_MODE` joins the `--help` Environment block.
...
- **Zed** (`install zed`, user and project scope): `context_servers` in `settings.json` -- `~/.config/zed/settings.json` on macOS and Linux (under `$XDG_CONFIG_HOME` on Linux when that is an absolute path), `%APPDATA%\Zed\settings.json` on Windows, `<project>/.zed/settings.json` at `--scope project`. Zed starts a server when the file is saved, so the Done line says no restart is needed, and a re-run without `--force` keeps the `enabled`, `remote` and `timeout` fields Zed owns on the entry.
- **Cline** (`install cline`, user scope only): `mcpServers` in `cline_mcp_settings.json`, which Cline reads as strict JSON. install writes the shared copy in `~/.cline/data/settings/` -- or wherever `CLINE_MCP_SETTINGS_PATH`, `CLINE_DATA_DIR` or `CLINE_DIR` puts it -- plus the copy under `User/globalStorage/saoudrizwan.claude-dev/settings/` of each of VS Code, VS Code Insiders, VSCodium, Cursor and Windsurf whose Cline storage directory exists, and uninstall takes the entry out of all of them. Cline picks the file up without a restart, and a re-run without `--force` keeps its `disabled`, `autoApprove` and `timeout` fields and a `"type": "stdio"`.
- **Continue** (`install continue`, user and project scope): a file of yaw-mcp's own, `mcpServers/yaw-mcp.json`, under `~/.continue` (or `CONTINUE_GLOBAL_DIR`) or `<project>/.continue`. The Done line says to reload the IDE window. On Windows the entry is bare `npx`, because Continue adds the `cmd.exe` wrapper itself.
- **Codex CLI** (`install codex-cli`, user and project scope): an `[mcp_servers.mcp]` table in `config.toml` under `CODEX_HOME` (default `~/.codex`), or `<project>/.codex/config.toml`, with `startup_timeout_sec` set to 60 seconds against Codex's own default of 10. On Windows the entry is bare `npx`, which needs Codex 0.59.0 or newer. A re-run without `--force` keeps `env_vars` and `enabled`; the Done line says to restart Codex.

Two gaps ship with them. `install --list` and `doctor` still read every client file as JSON, so a Codex CLI `config.toml` -- the one `install codex-cli` just wrote included -- shows as `malformed` in `install --list`, and `doctor` warns about it and exits 2 -- 1.0.5 above closes that one. And both report Cline by its shared file alone, not by the editor copies.

**`mcp`** is Claude Code's project scope under a second name: `install mcp`, `uninstall mcp` and `import mcp` act on the project-root `.mcp.json` exactly as `claude-code --scope project` does, and an explicit `--scope` beside it still wins. It is an alias rather than a row, so `install --list` and `doctor` do not show that file twice, and `try` does not take it.

**Fixed -- `doctor` stops offering steps `install` refuses, and stops asking you to remove by hand what `install` removes itself**

`yaw-mcp doctor` no longer offers a step `install` refuses. A client config that does not parse used to get "fix or rerun `yaw-mcp install`", and a rerun -- `--force` included -- exits 1 on that very file. Doctor, install and `import --remove-originals` now give one remedy from one helper: fix the JSON by hand or move the file aside, then run `yaw-mcp install <client>` (with `--scope` on a non-user row). A container key holding a non-empty array, which install also refuses, is named with its shape in doctor's row -- `present, but "mcpServers" is an array of 2, not a JSON object` -- instead of reading as "present, no entry -- run install", and `doctor --json` carries it in a new additive field, `clients[].containerBlocked`. That row is where the shape is named: `install --list` still reports such a file by the count in the list it reads -- `no-entries` for a blocked container, `malformed` for a file that does not parse at all. Doctor's exit code is unchanged by either: a file that does not parse was already a warning (exit 2), and a blocked container is a row line rather than a warning (exit 0 when nothing else is wrong).

`import --remove-originals` names an unparseable, non-object, blocked or directory-shaped client file for what it is rather than as "no yaw-mcp entry", and leads with install's by-hand step when that is the file install would write.

And doctor stops telling you to remove by hand a legacy `"mcp.hosting"` entry that `install` removes in the same write -- on the line for a lone legacy entry, and on all three lines for a yaw-mcp entry that cannot start (a missing launch command, a missing oam entry file, a bare `oam` command), which now say install removes it. The bare-`oam` line no longer suggests `OAM_BIN` as a second remedy either: the variable steers which binary `install` resolves and writes, never what the client spawns, so it cannot bring the working entry back on its own. It is now named as a precondition of that rerun -- `(set OAM_BIN to oam's full path first if install cannot find it)` -- which pairs the variable with a re-run the way install's own runtime lines do, though not in the same words: those read `Set OAM_BIN to oam's full path and re-run install to host yaw-mcp on it.` The two lines that name no install run (an entry that already works, and one whose launch path is for another OS) still say only that the key has to go.

**Fixed -- `--help` stops listing `servers`, the command 1.0.0 deleted**

1.0.0 deleted `yaw-mcp servers`, but the `--help` of both 1.0.0 and 1.0.1 still listed it under Inspection, as a deprecated command that "always fails" and pointed at `list`. That described the stub the release had removed; there was no command left behind the line. The entry is gone. Typing `yaw-mcp servers` is unchanged: it still prints the retired-verb hint naming `yaw-mcp list` and exits 2. A test now fails when the first word of any alias in a `--help` entry, or the first `yaw-mcp` command in a Quickstart step, is not a command yaw-mcp dispatches. Only that word is checked: not an action, flag or argument after it (the `install` in `sidecars install`, the `--list` in `install --list`), not a later `yaw-mcp` command in the same Quickstart step, and not a command named in a description or in the prose around the entries.

**Changed -- `release.sh` moves the oam floor to the latest oam release**

`MIN_OAM_VERSION` tracks the latest oam release as policy, and moving it was a manual step a release could skip. `release.sh` now reads the latest oam release from GitHub before its confirm prompt, sending the GitHub token its MCP-registry step resolves when there is one and retrying without it if that read fails. When the floor is behind and the version being released is not yet tagged or on npm, it moves the constant and the test literal that pins it, adds a block saying so at the end of the changelog section the release ships under -- or, when that section already holds a floor block the most recent release tag's CHANGELOG does not, rewrites that block instead; a floor block that tag already shipped is left as it is -- and commits those three files before lint, typecheck and tests run, so the gates run over the new floor. Once the version is tagged or on npm, the floor stays where it is and the run says so. Until then, a release stops when the floor is ahead of the latest oam release, and when GitHub cannot be read unless `ALLOW_STALE_OAM_FLOOR=1` is set. A re-run after a failure before the push carries on over the floor commit the failed run left on local main, as long as nothing else was committed on top of it and origin/main has not moved; otherwise the run stops, names `git reset --keep origin/main`, and lists any other commit that reset would discard. `release.sh` is not part of the published package.

**Changed -- the oam floor moves to 0.15.2**

`MIN_OAM_VERSION` tracks the latest oam release as policy, and v0.15.2 is now current (published 2026-09-13); the floor was 0.13.1. A machine whose oam is older hosts its node/npx sidecars on node instead, and logs a warning naming both versions, `oam self-update` as the fix, and that yaw-mcp needs a restart afterwards. `release.sh` moved the floor and wrote this block; it did not re-run the oam hosting check that `src/oam-spawn.ts` describes.

## 1.0.1 -- no behaviour change: what the npm page says about the package, and a lint gate that does not ship

Published 2026-09-12, a day after 1.0.0, and nothing a user runs is different. `src/` is byte-identical across the two tags -- zero lines of difference. Five of the published tarball's eight files still differ, and none of them is source: two of the three under `dist/` carry the version string `tsup` substitutes for `__VERSION__` at build time -- `index.js` five times, the shared chunk twice -- while the third, a 461-byte `auto-upgrade-*.js` re-export barrel, carries it nowhere and differs by the single line that names the chunk it imports. The two hashed files are both renamed and `index.js` is not, because tsup hashes a chunk's name from its content: the version string is in the chunk's content, and the barrel's content names the chunk. So that one substitution is the only difference anywhere under `dist/` that is not a file name. `README.md` carries the badge below; and `package.json`, which npm packs whatever the `files` allow-list says, carries the rewritten description, the `homepage` trailing slash, the thirteen added keywords and the two rewritten lint scripts. `CHANGELOG.md`, `LICENSE.md` and the schema are byte-identical. That `package.json` metadata is not an aside to the registry-page change the next block describes -- it is the mechanism for it. `git log --oneline v1.0.0..v1.0.1` prints five lines: the four changes this section covers, plus the release commit that bumps the version. Two of the four are observable, both on the registry page rather than in the tool; a third leaves nothing in the package but those two lint-script strings, which nothing shipped reads.

**Changed -- the description, keywords and homepage npm shows for this package**

The description was written for someone who already knew what yaw-mcp was ("One install, every MCP server, managed from yaw.sh/mcp. Runs locally: no account, no sign-in.") and now leads with the category and the clients ("MCP gateway and orchestrator: one local install fronts every MCP server for Claude Code, Cursor, and VS Code, loading tools on demand."). Thirteen keywords were added and none removed -- the shapes people search for (`mcp-gateway`, `mcp-proxy`, `mcp-broker`, `mcp-manager`), the clients (`claude-code`, `claude-desktop`, `cursor`, `vscode`, `windsurf`, `gemini-cli`), two capabilities the README already documents (`secrets-vault`, `lazy-loading`), and `cli`. That is all thirteen, and the array goes from 8 entries to 21. `orchestrator` shows up as a deletion in the diff and is not one: it is re-added further down the array. `homepage` gained a trailing slash to point at the per-server page, `https://yaw.sh/mcp/`.

`server.json` was not touched by that commit, so the MCP registry listing still carries the 1.0.0 wording while npm carries the new one: the registry's latest entry for `io.github.YawLabs/mcp` is 1.0.1 with the old description on it. `release.sh` bumps that file's version on every release, which is why the bump alone did not settle it -- the two keep disagreeing until a release carries the new wording into `server.json` as well.

The README gained an X follow badge above the tagline. `README.md` is in the package's `files` array, so it ships and is on the npm page; it is cosmetic either way.

**Changed -- contributor-only: `npm run lint` routes around a per-version biome crash**

Lint and `lint:fix` now go through `scripts/lint.mjs`. Everywhere except Windows ARM64 it is a passthrough to the platform binary; on that host it provisions `@biomejs/cli-win32-x64` into `node_modules/.cache` and runs it under emulation. Biome's own arguments are unchanged. The version to provision is read from the biome the repo actually installed -- `package-lock.json`, falling back to `node_modules/@biomejs/biome/package.json` -- and not from `biome.json`'s `$schema`, which pins the schema the config is validated against rather than the binary npm resolved; the two had drifted a minor apart on a sibling repo, where the stale version passed a tree the installed one rejects. Both read 2.4.16 here, so it is a no-op in this repo and a guard against the next bump. This is the same crash 0.72.0 pinned `@biomejs/biome` to 2.4.16 to avoid, and it is per-version and check-shaped rather than a standing ARM64 defect: 2.5.4 dies with 139 on a check run while answering `--version` with exit 0, and 2.4.16 and 2.5.13 both run correctly. `release.sh` had the attribution right already: at 1.0.0 its `SKIP_LINT` comments blamed the win32-arm64 biome binary and recorded that "the direct node_modules binary all died with zero output", with npm out of the picture. The warning against conflating that binary crash with npm's own wrapper segfault lives in a different comment -- the one covering the ARM64 exit-code tolerance, which calls the wrapper crash "distinct from the biome-2.5.x binary crash" and says "do not merge the two". Among what 1.0.1 changed in the `SKIP_LINT` comments: the shape of the claim -- "on every input" becomes per-version and check-shaped, with `--version` exiting 0 called out as not being evidence the gate works; the npm exclusion, until then carried only by that direct-binary measurement, is stated outright; the comments point at `scripts/lint.mjs` as the route that should make the variable unnecessary; and the consequence 1.0.0 already gave, that formatting "goes unverified whenever it is set", gains the reason it matters: this repo has no CI, so a `SKIP_LINT=1` release publishes unlinted. The behaviour change beside it is that a lint crash now fails the release outright, where before it was eligible for that same host segfault tolerance.

Almost none of that is in the published package. `files` is `dist`, `schemas`, `README.md` and `CHANGELOG.md`, so neither `scripts/lint.mjs` nor `release.sh` ships, and with `src/` untouched the shipped code is the 1.0.0 code but for the version string. What does ship is the `scripts.lint` and `scripts.lint:fix` strings in the packed `package.json` -- npm packs that file whatever `files` says -- now naming a `scripts/lint.mjs` the tarball does not contain. They were equally inert before, naming a `biome` binary an install of this package does not bring either, and nothing in the package runs them.

This entry is late. 1.0.1 published with no section of its own, under a `CHANGELOG.md` whose newest heading still read `## Unreleased` while describing what had already shipped as 1.0.0 -- so a reader on npm could not tell "nothing shipped" from "nobody wrote it down". Both halves are fixed now: that section is headed `## 1.0.0` below, and this is the release it was hiding.
...
**There is a way back: `YAW_MCP_CONFIG_RELOAD=0`** (or `=false`, the two spellings `YAW_MCP_AUTO_UPGRADE` already takes) restores the old behaviour, where an edit takes effect only after a client restart. This is the one change in this release that alters what an existing install does with nothing in your own config having moved, and that is the shape that ships with an off switch or does not ship. Setting it restores the previous behaviour *exactly* rather than approximately: the check sits ahead of even the file stat, so nothing is read and no fingerprint is recorded -- which means unsetting it mid-session picks up whatever the file says at the next boundary, instead of treating the edits you made while it was off as already-seen. `yaw-mcp doctor` lists it alongside the other knobs.
...
**Added -- `YAW_MCP_PREWARM=0`, for an upstream that cannot be started twice**

At startup yaw-mcp spawns any enabled server whose tool list it does not already know, reads that list, and closes the child again -- which is why a server you toggle on shows its tools without your having to load it, and why it costs nothing on later sessions: the list is persisted and re-learned only once it ages out. What that means for the one session that *does* learn a server is that the server starts twice: once to be read, once for the `activate` you actually asked for. Measured with six servers on a cold cache, every one of them spawned twice.

For almost every server that is a spawn nobody notices. For one whose startup is not idempotent -- it takes a lock file, binds a port, opens a database session, writes a login audit event -- it is that side effect happening twice, with the second one being the real load. `YAW_MCP_PREWARM=0` (or `=false`, the same two spellings `YAW_MCP_AUTO_UPGRADE` and `YAW_MCP_CONFIG_RELOAD` take) turns the pre-warm off: the check sits ahead of the scan, so nothing is looked at and nothing is spawned. The cost of turning it off is stated rather than hidden, in the docs and in one log line at startup -- a server whose tools have never been learned advertises none of them in `tools/list` until you activate it, which is the disappearing-server problem pre-warm exists to solve. `discover` still lists it, and activating it by name still works. `yaw-mcp doctor` shows the variable with the rest.

Nothing changes for an install that does not set it. Pre-warming stays on, because the thing it buys is real.

One related behaviour is now written down rather than changed. `YAW_MCP_SERVER_CAP` bounds servers that are *loaded* -- connected and advertising tools into the model's context, which is the budget it defends -- and pre-warm is exempt from it in both directions, so on a first session the number of live child processes briefly exceeds the cap. The excess is bounded, and by the pre-warm batch size rather than by the cap: pre-warm learns tool lists three servers at a time, so at most three extra children are alive at once no matter how many servers are configured, and each closes as soon as its tool list is read. With a cap of 6 and seven servers configured that shows up as a momentary seven live children while the seventh activation is still correctly refused. That is deliberate, but the cap's own description said "how many MCP servers can be simultaneously loaded" and left you to assume it bounded processes. It now says what it does and does not bound, and points at `YAW_MCP_PREWARM` for anyone who needs the process count itself held down.
...
A third surface reports that same upstream quantity -- the stderr warning the result cap writes when it cuts something -- and it goes through the same measuring function to get it, so the number in the log and the number in `mcp_connect_health` cannot be measured two different ways for one call. They can still differ, in exactly two places, and both are visible rather than mysterious. The cap sees the body *after* pruning, so on a result the pruner shrank its total is the smaller one. And the cap never sees `structuredContent` at all, so the warning names those bytes beside its own total as `bytesStructuredUncapped` rather than folding them into it -- inside, they would falsify both the ceiling printed next to them and the sentence the model reads, neither of which that payload was ever measured against.
...
**Changed -- `yaw-mcp list` exits 1 on a `bundles.json` it cannot read**

Three commands looked at the same unreadable `bundles.json` and reported three different exit codes. `list` printed `Could not read <path> -- yaw-mcp has nothing to load from it.` and exited **0**; `status` printed `UNREADABLE` and exited 1; `doctor` printed `Warnings above need attention.` and exited 2. Every one of the three diagnostics was correct and readable -- only the codes disagreed, which is the half nobody reads until a script is gating on it. `list` was the dangerous one: a caller branching on it saw SUCCESS over a config yaw-mcp cannot load, with zero servers available, which is indistinguishable from a healthy machine that simply has no servers yet.

`list` now exits **1**, in both text and `--json` mode. That is not a new convention, it is the one already written down: 2 is the argv/usage code the shared dispatch tail in `index.ts` emits, 1 is a generic failure, and 1 is specifically what `status` documents for this exact condition -- "a bundles.json is present but could not be read or parsed, so we cannot say what this machine loads" -- and what `sidecars install` already returned for its own `Could not read <path>` branch. `doctor` keeps its 2 and is not wrong to: its documented gate is unconditionally "any warning is a 2", and 0.77.0 changed its text path from 0 to 2 on purpose so that a script could key on it.

Nothing else moves. A machine with no `bundles.json` at all still exits 0 from `list` -- a fresh install is not a fault -- and `--json` still emits its document, warnings included, because a machine consumer reads the reason from `warnings` and the exit code only tells it whether to bother. The `--json` and text paths now agree, which they did not before.

**This changes what an existing script sees, with nothing of yours having moved**, so it is called out rather than filed under smaller things: `yaw-mcp list` returns 1 where it returned 0, for a broken config file only. There is no off switch, matching how this project handled the same shape twice already -- `doctor`'s text path going 0 to 2 in 0.77.0, and `yaw-mcp servers` going 1 to 2 above. An exit code that reports the wrong outcome is a defect, and preserving it behind a flag would mean shipping a documented way to keep being told a broken machine is fine.
