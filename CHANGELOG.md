...
The values are now typed on the one-shot page the vault prompt uses, served on `127.0.0.1`: one masked field per missing key, labelled with the key's name, with the same hardening and the same 3-minute expiry. A client that declares URL-mode elicitation gets a URL-mode prompt for the page, and is sent `notifications/elicitation/complete` when the page is done. A client with form mode only gets a form with no fields, as a yes/no consent, and on yes the broker opens the page in the default browser. The prompt names the server and the missing keys, and both it and the page say where to set them so the next session does not ask. While the page waits, the activation reports progress every 5 seconds, as the vault prompt does. What is typed is held in memory for the session and goes into that one server's env: never written to disk, never put in the vault passphrase's slot, and never passed to another server. Shutdown while the prompt is up refuses the activation with the usual `yaw-mcp is shutting down` message.

Each server still gets at most two asks per session. What ends the asking is new, and follows from what happened:

- A decline is a decision. No further ask for that server this session; this activate and every later one return the plain `Failed to load "<server>": ...` error.
- A page that cannot start, or a browser that cannot be opened, is a wall a second ask would hit identically, and ends the asking the same way. The activation fails with a message that says which of the two happened, that no further prompts are coming for that server this session, and to set the key in its `env` in bundles.json and activate again -- an edit picked up on the next `mcp_connect_*` call, with no client restart. Every later activate repeats it.
- An expired page spends its ask but does not end the asking: the user may simply have been away. The activation fails with a message saying the page expired with nothing submitted, and, while the server has an ask left, that activating again opens a new page; once the last ask is spent it says no further prompts are coming.
- The page cannot send an empty value (every field is required, and a POST with one missing or empty is answered 400 `Every field is required.`). A page result with no value in it is treated as a decline.

None of the three page failures is booked as a failed activation, which is what `discover`'s failure warning and `dispatch`'s down-ranking read: the server's only problem is a value nobody got to type.
...
**Fixed -- a cached server's tools are callable from a session's first request, with or without pre-warm**

The broker did not build its tool routing table at startup; the startup pre-warm's rebuild built it, as a side effect. Pre-warm returns before that rebuild when nothing is dormant -- every learned tool list still fresh, the usual state once each server's list has been learned -- and when `YAW_MCP_PREWARM=0`. Until something else rebuilt the table (an explicit activation, say), an `mcp_connect_exec` step naming a cached server's tool, or a `tools/call` on a cached tool that `tools/list` advertises under `YAW_MCP_TOOL_EXPOSURE=full`, answered `Unknown tool: <name>` instead of loading the server. The table is now built at startup, before the transport connects, so no `list_changed` notification is sent for it.
