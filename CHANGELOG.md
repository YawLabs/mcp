...
`uninstall`, `doctor`, `install --list` and `import` now also SEE an entry under the other drive-letter spelling of the same directory, which is what an older version wrote. Each of them used to look under the one key its own arguments spelled, so a spelling that did not match the one on disk read as an absence -- and which spelling a run computes is decided by the SHELL that reported the working directory, not by how the command was typed. The same text on both sides comes apart on that alone. Measured on a 1.0.0 build: a bare `install claude-code --scope local`, run from a cmd.exe sitting in the project after `cd /d c:\repo`, wrote `projects["c:/repo"]`, because cmd.exe keeps the drive letter as typed, with or without `/d`; the identically typed `uninstall claude-code --scope local --force`, run from PowerShell in that same directory, looked under `projects["C:/repo"]`, because PowerShell reports an upper-case drive whichever case was typed. That run stripped the `permissions.allow` grant, printed "Claude Code no longer launches yaw-mcp", and left `projects["c:/repo"].mcpServers.mcp` in place -- still launching the broker for any session whose cwd carried the other spelling -- and a second run then reported "Nothing to do". (Both shells report the directory with backslashes, `c:\repo` and `C:\repo`; the forward slashes are the key's, which is that path with every `\` rewritten as `/`.) Naming the directory does not settle it either: `--project-dir c:/repo` and `--project-dir C:/repo` each pin the key that one run computes, so spelling it out helps only when the second run repeats the first one's spelling. `doctor` and `install --list` reported the project as having no yaw-mcp entry at all. `uninstall` now removes the entry from every drive-letter spelling of the same key in one write, and names the key on the lines for the spellings that are not the canonical one -- the canonical removal still reads `Removed the "mcp" entry.`, and a sibling adds ` under projects["c:/repo"]` -- so its closing all-clear is no longer printed over an entry the same run left behind under the other spelling of the drive letter. `doctor` and `install --list` report the entry and name the key it is under: the STATUS column can now read `installed (other drive case)`, with the key itself printed in a note under the table, since that is the only place a full path fits. A script matching that column exactly will see the new text for a project whose entry is under the other spelling. Only the drive letter is folded: two keys differing in any other character, and POSIX and UNC keys, are still separate projects. `doctor --json` carries the key in a new additive field, `clients[].entryProjectKey`.
