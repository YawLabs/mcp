...
`MIN_OAM_VERSION` tracks the latest oam release as policy, and moving it was a manual step a release could skip. `release.sh` now reads the latest oam release from GitHub before its confirm prompt. When a fresh release finds the floor behind, it moves the constant and the test literal that pins it, adds a block saying so at the end of the changelog section the release ships under, and commits those three files before lint, typecheck and tests run, so the gates run over the new floor. A resume never moves the floor. A fresh release stops when the floor is ahead of the latest oam release, and when GitHub cannot be read unless `ALLOW_STALE_OAM_FLOOR=1` is set. `release.sh` is not part of the published package.
